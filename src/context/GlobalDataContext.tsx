...
import type { AnimatedShipData, FlightPlan } from '../components/common/StarMap/types/mapTypes';
...
export const GlobalDataProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    const { status, sendJson, addMessageListener, removeMessageListener } = useGlobalWsContext();
    
    // --- State Definitions ---
    const [dashboardData, setDashboardData] = useState<DashboardPayload | null>(null);
    const [currentCXDashboardFilters, setCurrentCXDashboardFilters] = useState<DashboardFilter>({ range: '7D', exchange: 'IC1' });
    ...
    const fetchStorageData = useCallback(async () => {
        try {
            ...
            const res = await fetch("https://api.punoted.net/internal/storage/user_storage", {
            ...
            if (json.success && json.data) {
                ...
                const rawData = Array.isArray(json.data) ? json.data : Object.values(json.data);
     ...
     */
    const fetchDashboard = useCallback((partialFilters: Partial<DashboardFilter> = {}) => {
        setIsLoading(true);
            ...
            switch (msg.type) {
                ...
                case "FLIGHT_PLAN_UPDATE": {
                    ...
                    setAnimatedShipData(prev => 
                        prev.map(ship => ship.id === plan.shipid ? { ...ship, plan: { ...plan } } : ship)
                    );
                    break;
                }
                ...
                case "SHIP_DATA_UPDATE": {
                    ...
                    setAnimatedShipData(prev => {
                        ...
                        shipMap.set(shipId, {
                            ...(existing || { id: shipId, position: [0, 0], progress: 0, plan: null }),
                            ...shipUpdate
                        });
    ...
    }, [addMessageListener, removeMessageListener, fetchDashboard, currentCXDashboardFilters]);
