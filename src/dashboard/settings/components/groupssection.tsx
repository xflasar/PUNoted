...
const AcceptInviteDialog: React.FC<{
	open: boolean;
	groupName: string;
	onClose: () => void;
	onAccept: (perms: string[]) => void;
}> = ({ open, groupName, onClose, onAccept }) => {
...
}> = ({ open, currentPerms, onClose, onSave }) => {
...
const GroupDetailDialog: React.FC<{
	...
	useEffect(() => {
		if (open && group) {
			fetch(`${API_BASE}/datagroup/${group.id}/members`, { headers })
				.then((r) => r.json())
				.then((data) => {
					setMembers(data);
					const me = data.find(
	...
	useEffect(() => {
		if (!inviteUser || inviteUser.length < 2) {
			setUserOptions([]);
			return;
		}
		const timer = setTimeout(async () => {
			...
			try {
				const res = await fetch(
					`${API_BASE}/datagroup/users/search?q=${inviteUser}`,
					{ headers },
				);
	...
	const handleInvite = async () => {
		...
		try {
			const res = await fetch(`${API_BASE}/datagroup/${group.id}/invite`, {
				method: "POST",
				headers: { ...headers, "Content-Type": "application/json" },
				body: JSON.stringify({ username: inviteUser }),
			});
			if (res.ok) {
				showSnackbar(`Invited ${inviteUser}`, "success");
				setInviteUser("");
				setUserOptions([]);
			} else {
				const err = await res.json();
				showSnackbar(err.detail || "Failed", "error");
			}
		} catch {
	...
	const handleUpdateMyPerms = async (newPerms: string[]) => {
		if (!group) return;
		try {
			const res = await fetch(`${API_BASE}/datagroup/${group.id}/shares`, {
				method: "PATCH",
				headers: { ...headers, "Content-Type": "application/json" },
			...
			if (res.ok) {
				...
				const mRes = await fetch(`${API_BASE}/datagroup/${group.id}/members`, {
					headers,
				});
	...
	const toggleReadAccess = async (targetId: string, current: boolean) => {
		if (!group) return;
		try {
			await fetch(`${API_BASE}/datagroup/${group.id}/permissions`, {
				method: "POST",
				headers: { ...headers, "Content-Type": "application/json" },
				body: JSON.stringify({ username: targetId, can_read_data: !current }),
			});
			setMembers((prev) =>
				prev.map((m) =>
					m.user_id === targetId ? { ...m, can_read_data: !current } : m,
				),
			);
		} catch {
	...
	const handleKickMember = async (targetId: string) => {
		...
		try {
			const res = await fetch(
				`${API_BASE}/datagroup/${group.id}/members/${targetId}`,
	...
	const handleLeaveGroup = async () => {
		...
		try {
			const res = await fetch(`${API_BASE}/datagroup/${group.id}/leave`, {
				method: "DELETE",
				headers,
			});
			if (res.ok) {
	...
	const handleDeleteGroup = async () => {
		...
		try {
			const res = await fetch(`${API_BASE}/datagroup/${group.id}`, {
				method: "DELETE",
				headers,
			});
			if (res.ok) {
					...
					{group?.is_owner && (
						<Box sx={{ display: "flex", gap: 1, mb: 3 }}>
							<Autocomplete
								...
								inputValue={inviteUser}
								onInputChange={(e, val) => setInviteUser(val)}
							...
							/>
							<Button variant="contained" onClick={handleInvite} sx={{ px: 3 }}>
								Invite
							</Button>
						</Box>
					)}
...
const GroupsSection: React.FC<{
	...
	showSnackbar: (msg: string, type: any) => void;
	wsTrigger: number;
}> = ({ userId, headers, showSnackbar, wsTrigger }) => {
	const theme = useTheme();
	...
	const handleCreate = async (name: string, desc: string) => {
		try {
			const res = await fetch(`${API_BASE}/datagroup/`, {
				method: "POST",
				headers: { ...headers, "Content-Type": "application/json" },
	...
	const handleAcceptInvite = async (perms: string[]) => {
		const group = acceptInviteData.group;
		if (!group) return;
		try {
			const res = await fetch(`${API_BASE}/datagroup/${group.id}/accept`, {
				method: "POST",
				headers: { ...headers, "Content-Type": "application/json" },
			...
			showSnackbar("Error accepting", "error");
		}
	};
