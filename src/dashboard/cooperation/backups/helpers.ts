...
import {
	...
	ALLOY_TICKERS,
	CHEMICALS_TICKERS,
	CONSTRUCTION_MATERIALS_TICKERS,
	CONSTRUCTION_PARTS_TICKERS,
	CONSTRUCTION_PREFABS_TICKERS,
	CONSUMABLE_BASIC_TICKERS,
	CONSUMABLE_BUNDLES_TICKERS,
	CONSUMABLE_LUXURY_TICKERS,
	DRONES_TICKERS,
	ELECTRONIC_DEVICES_TICKERS,
	ELECTRONIC_PARTS_TICKERS,
	ELECTRONIC_PIECES_TICKERS,
	ELECTRONIC_SYSTEMS_TICKERS,
	ELEMENTS_TICKERS,
	ENERGY_SYSTEMS_TICKERS,
	FUELS_TICKERS,
	GASES_TICKERS,
	INFRASTRUCTURE_TICKERS,
	LIQUIDS_TICKERS,
	MEDICAL_EQUIPMENT_TICKERS,
	METALS_TICKERS,
	MINERALS_TICKERS,
	ORES_TICKERS,
	PLASTICS_TICKERS,
	SHIP_ENGINES_TICKERS,
	SHIP_KITS_TICKERS,
	SHIP_SHIELDS_TICKERS,
	SOFTWARE_COMPONENTS_TICKERS,
	SOFTWARE_SYSTEMS_TICKERS,
	SOFTWARE_TOOLS_TICKERS,
	TEXTILES_TICKERS,
	UNIT_PREFABS_TICKERS,
	UTILITY_TICKERS,
	SHIP_PARTS_TICKERS,
} from "./consts";
...
export const validateConnection = (
	...
	const targetMaterial = materials.find(
		(m) => m.ticker === targetNode.materialTicker,
	);
...
export const ConnectionToolContext =
	React.createContext<ConnectionToolContextProps>(defaultContextValue);
	...
	return chain.links
		...
		.map(
			(link: {
				...
				const material =
					materials?.find((m: { ticker: any }) => m.ticker === ticker) || null;
 ...
 */
export const getCalculatedFlowRate = (
	sourceId: string,
	ticker: string,
	nodes: ChainNodeData[],
) => {
 ...
 */
export const transformChainToRF = (
	chain: Chain | null | undefined,
	materials: Material[],
): { rfNodes: Node<ChainNodeData>[]; rfEdges: Edge<ChainLinkData>[] } => {
	...
	const rfEdges = buildTransshipmentEdges(
		chain,
		materials,
	) as unknown as Edge<ChainLinkData>[];
				...
				return data.data.map((m: any) => {
					...
					m.inputRecipes.forEach((recipe: any) => {
						recipe.inputs.forEach((input: any) =>
							inputMaterials.add(input.ticker),
						);
					});

					const production_building = m.resource
						? "Resource"
						: (m.category.length > 10
								? m.category.substring(0, 10)
...
const createMockUserOtherStorage = (
...
export const createMockUserStorage = (materialTicker: string, unit: string) => [
	{
		username: "User-Alpha",
		current: materialTicker === "FE" ? 120 : 50,
		capacity: 200,
		unit,
		overallStorage: createMockUserOtherStorage(
			materialTicker,
			unit,
			"User-Alpha",
		),
	...
	{
		username: "User-Beta",
		current: materialTicker === "FE" ? 80 : 10,
		capacity: 100,
		unit,
		overallStorage: createMockUserOtherStorage(
			materialTicker,
