...
import {
	aggregateNodeProduction,
	formatCurrency,
} from "../../components/basemanager/helpers";

const UserProductionList = React.memo(
	({ aggregatedData, theme }: { aggregatedData: any[]; theme: any }) => {
		return (
			<List
				dense
				disablePadding
							...
							>
								<Typography
									...
									sx={{
										...
										whiteSpace: "nowrap",
										fontWeight: data.isRegistered ? "bold" : "normal",
										color: data.isRegistered
...
const TransportList = React.memo(
	({ nodeData, theme }: { nodeData: ChainNodeData; theme: any }) => {
		const trans = nodeData.transport || {
			manualTons: 0,
			manualVolume: 0,
			userShips: [],
		};
			...
			>
				...
				>
					...
					<Box sx={{ display: "flex", gap: 2, mt: 0.5 }}>
						...
						>
							{Number(trans.manualTons || 0).toFixed(0)} t
						</Typography>
						<Typography
							variant="body2"
							sx={{
						...
						>
							{Number(trans.manualVolume || 0).toFixed(0)} m³
						</Typography>
					</Box>
				</Box>
				...
				<Box sx={{ flexGrow: 1, overflowY: "auto" }}>
					...
					<List dense disablePadding>
						{trans.userShips && trans.userShips.length > 0 ? (
							trans.userShips.map((ship: any, i: number) => (
								<ListItem
									key={i}
									disableGutters
									sx={{
								...
								>
									...
									>
										...
										<Typography
											...
											fontWeight="bold"
										>
											{Number(ship.tons).toFixed(0)}t /{" "}
											{Number(ship.volume).toFixed(0)}m³
										</Typography>
									</Box>
								</ListItem>
							))
								...
								>
									...
									>
										...
										>
											<StorageIcon
												fontSize="inherit"
												sx={{ color: theme.palette.info.light, mr: 0.5 }}
											/>
										...
										>
											...
											>
												{current.toFixed(0)} / {capacity.toFixed(0)}
											</Typography>
											{isExpanded ? (
...
const NodeInputsList = React.memo(
	({
		inputStatus,
		theme,
	}: {
		inputStatus: Record<string, any>;
		theme: any;
	}) => {
		const entries = Object.entries(inputStatus).sort(
					...
					entries.map(([ticker, data]) => {
						...
						return (
							...
							>
								...
								>
									<Typography
										sx={{ fontWeight: 500, width: "20%", fontSize: "0.85rem" }}
									>
										{ticker}
									</Typography>
									<Typography
...
const MinimalistFlowCard = React.memo(({ data, selected, isDragging }: any) => {
	...
	const displayNodeData = useMemo(
		() => ({ ...nodeData, userStorage: liveStorage ?? nodeData.userStorage }),
		[nodeData, liveStorage],
	);
	...
	const nodeAggregation = useMemo(() => {
		return aggregateNodeProduction(
			displayNodeData.assignedUsers || [],
			displayNodeData.materialTicker || "UNK",
			staticData,
			getPrice,
		);
	}, [
	...
	const netFlowColor =
		realNet >= 0 ? theme.palette.secondary.main : theme.palette.error.main;
		...
		<Card sx={nodeStyle}>
			...
			>
				<Box sx={{ display: "flex", alignItems: "center", overflow: "hidden" }}>
					...
					>
						{displayNodeData.materialTicker || "Group"}
					</Typography>
					<Typography
						variant="caption"
						sx={{
				...
				{displayNodeData.inputStatus &&
					Object.keys(displayNodeData.inputStatus).length > 0 && (
						...
						>
							...
							<Box sx={{ flexGrow: 1, overflowY: "auto" }}>
								<NodeInputsList
									inputStatus={displayNodeData.inputStatus}
									theme={theme}
								/>
							</Box>
						</Box>
				...
				>
					...
					<Box sx={{ flexGrow: 1, p: 0.5, overflowY: "auto" }}>
						{activeListTab === "Production" && (
							<UserProductionList
								aggregatedData={nodeAggregation.userProd}
								theme={theme}
							/>
						)}
						...
						{activeListTab === "Transport" && (
							<TransportList nodeData={displayNodeData} theme={theme} />
						)}
					</Box>
				</Box>
			</Box>
			...
			<Handle
				...
				id={targetHandleId}
				hidden={
					displayNodeData.nodeType === "Starter" || displayNodeData.isResource
				}
				style={{
					width: 10,
					height: 10,
			...
			<Handle
				...
				id={sourceHandleId}
				hidden={
					displayNodeData.nodeType === "End" || displayNodeData.isEndMaterial
				}
				style={{
					width: 10,
					height: 10,
