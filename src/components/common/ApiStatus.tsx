...
const StatusIndicator = styled(Box)(
	({ statusColor }: { statusColor: string }) => ({
		...
		height: 8,
		borderRadius: "50%",
		backgroundColor: statusColor, // Use dynamic color based on status
		boxShadow: `0 0 5px ${statusColor}`, // Use dynamic color for shadow
	})
);
...
const ApiStatus = ({ apiStatus = 'online' }: ApiStatusProps) => {
	...
	if (apiStatus === "online") {
		...
		statusColor = "#90ee90"; // Light green for online
	} else {
		statusText = "API STATUS: OFFLINE";
		statusColor = "red"; // Red for offline
	}
