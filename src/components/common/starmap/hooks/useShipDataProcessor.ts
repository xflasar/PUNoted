...
export const useShipDataProcessor = (
  ...
  return useMemo(() => {
    ...
    const otherGroupsMap = new Map<string, { name: string; ships: AnimatedShipData[] }>();
    ...
    const processShip = (ship: AnimatedShipData, isOwnedByData: boolean) => {
      const isMine = isOwnedByData || (currentUserId && String(ship.user_id) === String(currentUserId));
      const mapShip = { 
          ...ship, 
          visible: true,
          isOwn: isMine,
          addresssystemid: ship.addresssystemid || (ship as any).address_system_id,
          addressplanetid: ship.addressplanetid || (ship as any).address_planet_id,
          addressstationid: ship.addressstationid || (ship as any).address_station_id
      };

      // 1. Determine Dynamic State based on time (Is it flying or landed?)
      let isFlying = false;
      let arrivalMs = 0;
      if (mapShip.plan && mapShip.plan.segments && mapShip.plan.segments.length > 0) {
          ...
          const t2 = mapShip.plan.departuretimestamp ? new Date(mapShip.plan.departuretimestamp).getTime() : 0;
    ...
    Array.from(otherGroupsMap.entries()).sort().forEach(([_, group]) => {
    ...
    const isPlanActive = (s: AnimatedShipData) => {
      ...
      if (s.plan.arrivaltimestamp) {
        ...
        const t2 = s.plan.departuretimestamp ? new Date(s.plan.departuretimestamp).getTime() : 0;
    ...
    const myPlans = own
      .filter(s => visiblePathShipIds.has(s.ship_id || (s as any).id) && isPlanActive(s))
      .map(s => ({ ...s.plan!, isOwn: true, shipid: s.ship_id || (s as any).id }));
      
    const corpPlans = allCorpShips
      .filter(s => visiblePathShipIds.has(s.ship_id || (s as any).id) && isPlanActive(s))
      .map(s => ({ ...s.plan!, isOwn: false, shipid: s.ship_id || (s as any).id }));
