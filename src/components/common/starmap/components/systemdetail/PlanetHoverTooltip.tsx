...
import type { PlanetPosition, PlanetData, ShipData } from "../../types/maptypes";
...
const PlanetHoverTooltip: React.FC<PlanetHoverTooltipProps> = ({
	...
	const fullPlanetData = useMemo(() => {
		...
		return systemPlanets.find((p) => String(p.planetid) === String(planetId)) || null;
	}, [allPlanetsData, parentSystemId, planetId]);

	const planetPopulation = fullPlanetData?.planetPopulation ?? object.planetPopulation ?? 0;
	...
	const ownShipsCount = ownerShips.filter(
		(s) => String(s.address_planet_id || s.addressplanetid) === String(planetId)
	).length;
	...
	return (
		<Paper
			...
			sx={{
				...
				p: 1.5,
				borderRadius: "8px",
				background: "linear-gradient(135deg, rgba(10, 14, 26, 0.95) 0%, rgba(5, 7, 12, 0.98) 100%)",
		...
		>
			...
			<Box sx={{ display: "flex", flexDirection: "column", gap: 0.5 }}>
				<Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
					...
					<Typography variant="caption" sx={{ fontWeight: 700, fontSize: "0.7rem", color: theme.palette.text.primary }}>
						{planetPopulation.toLocaleString()}
					</Typography>
				</Box>

				{(ownShipsCount > 0 || otherShipsCount > 0) && (
					<Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderTop: "1px dashed rgba(255,255,255,0.06)", pt: 0.5, mt: 0.25 }}>
						<Typography variant="caption" sx={{ color: "rgba(255,255,255,0.5)", fontSize: "0.7rem", display: "flex", alignItems: "center", gap: 0.5 }}>
							<Sailing sx={{ fontSize: 12, opacity: 0.6 }} /> Docked Ships
						</Typography>
						<Typography variant="caption" sx={{ fontWeight: 700, fontSize: "0.7rem", color: "#00e5ff" }}>
							{ownShipsCount} Mine {otherShipsCount > 0 && `• ${otherShipsCount} Other`}
						</Typography>
					</Box>
				)}

				{resources.length > 0 && (
					<Box sx={{ borderTop: "1px dashed rgba(255,255,255,0.06)", pt: 0.75, mt: 0.25 }}>
						<Typography variant="caption" sx={{ color: "rgba(255,255,255,0.4)", display: "block", fontSize: "0.6rem", fontWeight: 700, mb: 0.5 }}>
							NATURAL RESOURCES
						</Typography>
						<Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.5 }}>
							{resources.map((r, idx) => {
								...
								return (
									<Box key={idx} sx={{ display: "inline-flex", alignItems: "center", bgcolor: "rgba(0, 0, 0, 0.2)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "4px", px: 0.5, py: 0.15, gap: 0.25 }}>
										<Box sx={{ fontSize: "0.6rem", display: "inline-flex" }}>
											<MaterialBadge ticker={ticker} />
										</Box>
										<Typography variant="caption" sx={{ fontSize: "0.6rem", fontWeight: 700, color: "rgba(255,255,255,0.8)" }}>
