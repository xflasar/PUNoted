...
import { Sailing, Business, LocationCity, TravelExplore } from "@mui/icons-material";
import type { MapPoint, PlanetData, StationData, ShipData } from "../../types/maptypes";
...
const SystemHoverTooltip: React.FC<SystemHoverTooltipProps> = ({
	...
	if (isStation) {
		...
		const ownDockedCount = ownerShips.filter(
			(s) => s.addressstationid === stationId || s.address_station_id === stationId
		).length;
				...
				<Box sx={{ display: "flex", flexDirection: "column", gap: 0.5 }}>
					...
					<Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
						...
						<Typography variant="caption" sx={{ fontWeight: 700, fontSize: "0.7rem", color: theme.palette.text.primary }}>
							{ownDockedCount + otherDockedCount > 0 ? `${ownDockedCount} Mine • ${otherDockedCount} Other` : "None"}
						</Typography>
					</Box>
				</Box>
			</Paper>
	...
	const isSystem = object.type === "system" || (!object.type && object.originalSystemId);
					...
					<Box sx={{ borderTop: "1px dashed rgba(255,255,255,0.06)", pt: 0.75, mt: 0.25 }}>
						...
						<Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.5 }}>
							{systemResources.map((ticker) => (
								<Box key={ticker} sx={{ fontSize: "0.65rem", display: "inline-flex" }}>
