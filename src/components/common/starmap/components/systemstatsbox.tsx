...
const PopulationLegend: React.FC<{ minPop: number; maxPop: number }> = ({
	minPop,
	maxPop,
}) => {
...
export const SystemStatsBox: React.FC<{ stats: SystemStats | null }> = ({
	stats,
}) => {
	...
	const getMinMaxPopulations = () => {
		...
		const populations = stats.planetDetails
			.map((p) => p.population)
			.filter((p) => p > 0);
	...
	const getSystemClassification = (population: number) => {
		...
		if (population >= 100000)
			return { label: "Established Colony", color: "warning.main" };
		if (population > 0)
			return { label: "Outpost / Frontier", color: "info.main" };
		return { label: "Unsettled", color: "text.disabled" };
	};
	const systemClassification = getSystemClassification(
		stats.totalSystemPopulation,
	);
					...
					<Box sx={{ mb: 1 }}>
						<Box sx={{ display: "flex", justifyContent: "space-between" }}>
							...
							<Typography
								variant="body2"
								fontWeight="bold"
								sx={{ color: systemClassification.color }}
							>
						...
						<Box sx={{ display: "flex", justifyContent: "space-between" }}>
							...
							<Typography variant="body2" fontWeight="bold">
								{stats.totalSystemPopulation.toLocaleString()}
							</Typography>
						</Box>
					</Box>
							...
							{stats.planetDetails.map((planet, index) => (
								<Box
									...
									sx={{
										...
										justifyContent: "space-between",
										borderBottom:
											index < stats.planetDetails.length - 1
								...
								>
									<Typography
										...
										color="text.secondary"
										noWrap
										sx={{ flexGrow: 1, textAlign: "left", mr: 1 }}
									>
										{planet.name}
									</Typography>
									<Typography
