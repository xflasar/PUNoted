...
import { Box, TextField, Autocomplete, Typography, Divider, Switch, ToggleButtonGroup, ToggleButton, Select, MenuItem, Button } from '@mui/material';
...
const FilterPanel: React.FC = () => {
	...
	const systemOptions = useMemo(() => {
		...
		return systemsPoints.map((s: any) => ({
	...
	const selectedOriginOption = useMemo(() => {
		return systemOptions.find((opt) => opt.id === filter.originSystemId) || null;
	}, [systemOptions, filter.originSystemId]);

	const selectedDestinationOption = useMemo(() => {
		return systemOptions.find((opt) => opt.id === filter.destinationSystemId) || null;
	}, [systemOptions, filter.destinationSystemId]);

	const updateFilterField = <K extends keyof FilterState>(field: K, value: FilterState[K]) => {
		setFilter((prev) => ({
			...prev,
			[field]: value,
		}));
	...
	return (
		<Box sx={{
			...
			<Box sx={{
				...
				<Typography variant="subtitle2" sx={{ fontWeight: 800, color: '#00e5ff', textTransform: 'uppercase', fontSize: '0.7rem', letterSpacing: '0.15em', textShadow: '0 0 8px rgba(0, 229, 255, 0.4)' }}>
					Map Filters
				</Typography>
				<Button
			...
			<Box sx={{
				...
				{/* Planet Type Toggles */}
				<Box>
					<Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.6)', fontWeight: 600, fontSize: '0.7rem', display: 'block', mb: 0.5 }}>Planet Type</Typography>
					<ToggleButtonGroup
				...
				{/* Fertility Toggle */}
				<Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', bgcolor: 'rgba(0,0,0,0.2)', p: 1, borderRadius: '6px', border: '1px solid rgba(255,255,255,0.05)' }}>
					<Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.8)', fontWeight: 600, fontSize: '0.75rem' }}>Fertile Planets Only</Typography>
					<Switch
				...
				<Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
					{/* Gravity */}
					<Box>
						<Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.6)', fontWeight: 600, fontSize: '0.7rem', display: 'block', mb: 0.5 }}>Gravity</Typography>
						<ToggleButtonGroup
					...
					{/* Temperature */}
					<Box>
						<Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.6)', fontWeight: 600, fontSize: '0.7rem', display: 'block', mb: 0.5 }}>Temperature</Typography>
						<ToggleButtonGroup
							value={filter.temperature}
							exclusive
							onChange={(_, val) => val && updateFilterField('temperature', val)}
							size="small"
							fullWidth
							sx={{
								bgcolor: 'rgba(0,0,0,0.3)',
					...
					{/* Pressure */}
					<Box>
						<Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.6)', fontWeight: 600, fontSize: '0.7rem', display: 'block', mb: 0.5 }}>Pressure</Typography>
						<ToggleButtonGroup
				...
				<Box sx={{ bgcolor: 'rgba(0,0,0,0.2)', p: 1.5, borderRadius: '8px', border: '1px solid rgba(255,255,255,0.06)' }}>
					<Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 1 }}>
						<Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.8)', fontWeight: 600, fontSize: '0.75rem' }}>Filter COGC Program</Typography>
						<Switch
							checked={filter.cogcEnabled}
							onChange={(e) => updateFilterField('cogcEnabled', e.target.checked)}
							size="small"
							sx={{
								'& .MuiSwitch-switchBase.Mui-checked': { color: '#00e5ff' },
								'& .MuiSwitch-switchBase.Mui-checked + .MuiSwitch-track': { backgroundColor: '#00e5ff' }
					...
					{filter.cogcEnabled && (
							...
							MenuProps={{
								PaperProps: {
									sx: {
										...
										'& .MuiMenuItem-root': {
											fontSize: '0.75rem',
											'&.Mui-selected': { bgcolor: 'rgba(0, 229, 255, 0.15) !important' },
						...
						>
							...
							<MenuItem value="RESOURCE_EXTRACTION">Resource Extraction</MenuItem>
				...
				<Box>
					<Typography variant="caption" sx={{ display: 'block', mb: 1, color: 'rgba(255,255,255,0.6)', fontWeight: 600, fontSize: '0.7rem' }}>Resource Filters</Typography>
					<Box sx={{
						display: 'flex',
						flexWrap: 'wrap',
						gap: 0.75,
							...
							return (
								<Box
									...
									sx={{
										cursor: 'pointer',
										borderRadius: '4px',
										border: isActive ? '1px solid #00e5ff' : '1px solid rgba(255,255,255,0.12)',
										...
										'&:hover': {
											...
											opacity: 1,
											borderColor: isActive ? '#00e5ff' : 'rgba(255,255,255,0.3)',
										}
									}}
								>
				...
				<Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
					<Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.6)', fontWeight: 600, fontSize: '0.7rem', display: 'block', mb: -0.5 }}>Path Finding</Typography>
					...
					<Box>
						<Autocomplete
							...
							onChange={(_, val) => updateFilterField('originSystemId', val ? val.id : null)}
							renderInput={(params) => (
								<TextField
									{...params}
									variant="standard"
									...
									sx={{
										...
										'& .MuiInput-underline:hover:before': { borderBottomColor: 'rgba(255,255,255,0.25) !important' },
							...
							PaperComponent={({ children, ...other }) => (
								<Paper {...other} sx={{
									...
									'& .MuiAutocomplete-option': {
										fontSize: '0.75rem',
										'&[aria-selected="true"]': { bgcolor: 'rgba(0, 229, 255, 0.15) !important' },
										'&:hover': { bgcolor: 'rgba(255, 255, 255, 0.05) !important' },
									}
								}}>
									{children}
					...
					<Box>
						<Autocomplete
							...
							onChange={(_, val) => updateFilterField('destinationSystemId', val ? val.id : null)}
							renderInput={(params) => (
								<TextField
									{...params}
									variant="standard"
				...
				{/* Radius Setting */}
				<Box>
					<Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.6)', fontWeight: 600, fontSize: '0.7rem' }}>Radius Limit (parsecs)</Typography>
					<TextField
						type="number"
						variant="standard"
						fullWidth
						value={filter.filterRadius}
						onChange={(e) => updateFilterField('filterRadius', Math.max(0, Number(e.target.value)))}
						InputProps={{
