		...
		<Box sx={{ mb: 1 }}>
			<Box sx={{ display: "flex", justifyContent: "space-between" }}>
				...
				<Typography variant="caption">
					{resource.value.toFixed(0)} u/day
				</Typography>
			</Box>
			<LinearProgress
				variant="determinate"
				...
				<Box sx={{ overflowY: "auto", p: 1 }}>
					{systems.length > 0 && (
						<Accordion
							...
							elevation={0}
							defaultExpanded
							sx={{ backgroundImage: "none", background: "transparent" }}
						>
							<AccordionSummary
							...
							<AccordionDetails sx={{ p: 0.5 }}>
								<List dense>
									{systems.map((system) => (
										<Card
											key={system.id}
											sx={{ mb: 1, background: "rgba(0,0,0,0.2)" }}
										>
											<CardContent
												onClick={() =>
													handleResultClick(system.x, system.y, system.id)
												}
												sx={{
													cursor: "pointer",
													p: 1,
								...
								<List dense>
									{planets.map((planet) => (
										...
										>
											...
											<Collapse
												in={expandedResult === planet.planetid}
												timeout={150}
											>
												<CardContent sx={{ p: 1.5, pt: 0 }}>
													<Typography variant="caption">
														System: {planet.systemInfo.label}
													</Typography>
													<Typography variant="caption" sx={{ ml: 1 }}>
														Population:{" "}
														{planet.planetPopulation?.toLocaleString() ?? "N/A"}
													</Typography>
													<Typography
														variant="subtitle2"
														sx={{ mt: 1, mb: 0.5 }}
