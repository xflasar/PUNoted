...
import type { WorkerFlightPlan, MapPoint, PlanetData, StationData, AnimatedShipData } from "../types/mapTypes";

  export const findSystemById = (systemsPoints : MapPoint[]) => (systemId: string) => {
      return systemsPoints.find((p: MapPoint) => p.originalSystemId === systemId);
    }

  export const findPlanetById = (allPlanetsData: Record<string, PlanetData[]>) => (systemId: string, planetId: string) => {
      const planetsInSystem = allPlanetsData[systemId];
      const planet = planetsInSystem?.find((p: PlanetData) => p.planetid === planetId);
      return planet ? planet.planetname : null;
    }

  export const findStationById = (allStationsData: Record<string, StationData[]>) => (systemId: string, stationId: string) => {
      const stationsInSystem = allStationsData[systemId];
      const station = stationsInSystem?.find((s: StationData) => s.stationid === stationId);
      return station ? station.name : null;
    }

  export const getOriginDestinationLabel = (systemPoints: MapPoint[], allPlanetsData: Record<string, PlanetData[]>, allStationsData: Record<string, StationData[]>) => 
    (flightPlan: WorkerFlightPlan, isOrigin = true) => {
        ...
        if (planetId && systemId) specificLabel = findPlanetById(allPlanetsData)(systemId, planetId);
        else if (stationId && systemId) specificLabel = findStationById(allStationsData)(systemId, stationId);
        
        const systemLabel = systemId ? findSystemById(systemPoints)(systemId)?.label : undefined;
