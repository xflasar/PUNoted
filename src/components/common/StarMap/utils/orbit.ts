...
export function calculateOrbitEllipse(
  ...
  for (let j = 0; j <= numSegments; j++) {
    ...
    const inertial = orbitalPlaneToInertial([yOrb, xOrb, zOrb], raan_rad, aop_rad, i_rad);
 ...
 * Rz(-Ω) * Rx(-i) * Rz(-ω) * x
 */
export function orbitalPlaneToInertial(x: number[], Ω: number, ω: number, i: number): number[] {
  // Rz(-Ω)
