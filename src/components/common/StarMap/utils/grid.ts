...
function getCacheKey(systems: MapPoint[], edges: Edge[], opts: any): string {
    ...
    const systemIds = systems.map(s => s.originalSystemId || s.id).sort().join('|');

    // 2. Collect sorted edge definitions (origin-destination)
    const edgeDefs = edges.map(e => [e.systemidorigin, e.systemiddestination].sort().join('-')).sort().join('|');
...
export async function resolveInitialOverlapsAsync(
  systems: MapPoint[],
  edges: Edge[],
  opts?: { cellSize?: number; minSeparation?: number; maxIterations?: number; minLineSeparation?: number }
): Promise<{ adjusted: MapPoint[]; bounds: { minX: number; minY: number; maxX: number; maxY: number } }> {
  ...
  const finalOpts = { cellSize, minSeparation, maxIterations, minLineSeparation };
  ...
  try {
    ...
    if (cachedData) {
      ...
      if (parsed.adjusted && parsed.bounds && parsed.adjusted.length === systems.length) {
        return parsed;
      }
    }
  } catch (e) {
  ...
  try {
    ...
    const result = await new Promise<{ adjusted: MapPoint[]; bounds: any }>((resolve, reject) => {
    ...
    for (let iter = 0; iter < maxIterations; iter++) {
      ...
      const currentEdges = edgesWithCoords.map(e => {
        const origin = adjusted.find(s => s.originalSystemId === e.systemidorigin);
        const destination = adjusted.find(s => s.originalSystemId === e.systemiddestination);
        return { ...e, ax: origin?.x ?? e.ax, ay: origin?.y ?? e.ay, bx: destination?.x ?? e.bx, by: destination?.y ?? e.by };
      });
...
export function buildSystemConnections(connections: Edge[], systems: MapPoint[]) {
  const systemMap = new Map<string, MapPoint>();
  for (const s of systems) if (s.originalSystemId) systemMap.set(s.originalSystemId, s);
  const edges = [];
  for (const c of connections) {
    const origin = systemMap.get(c.systemidorigin);
    const destination = systemMap.get(c.systemiddestination);
    if (origin && destination) edges.push({ sourcePosition: [origin.x, origin.y], targetPosition: [destination.x, destination.y] });
  }
  return edges;
}