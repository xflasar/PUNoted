...
function _calculateDistancesOnEllipse(tA: number, tB: number) {
  ...
  let sign = 1;
  if (delta < 0) { sign = -1; delta = -delta; } // minDistanceSign
 ...
 */
function distanceOnEllipse(ellipse: {
  center: Vec3, semiMajorAxis: number, semiMinorAxis: number, alpha: number
}, start: Vec3, end: Vec3) {
  // matches the game's: rotate point by +alpha into ellipse-local space
  const r = start.clone().sub(ellipse.center).applyAxisAngle(Z_AXIS, ellipse.alpha);
  const o = end.clone().sub(ellipse.center).applyAxisAngle(Z_AXIS, ellipse.alpha);

  const tA = _findTForPointOnEllipse(ellipse.semiMajorAxis, ellipse.semiMinorAxis, r);
  const tB = _findTForPointOnEllipse(ellipse.semiMajorAxis, ellipse.semiMinorAxis, o);
 ...
 */
function waypointsOnEllipse(ellipse: {
  center: Vec3, semiMajorAxis: number, semiMinorAxis: number, alpha: number
}, start: Vec3, end: Vec3) : Vec3[] {
  ...
  for (let a = 0; a < steps; a++) {
    ...
    const l = _calculateEllipseAt(new Vec3(), s, ellipse.semiMajorAxis, ellipse.semiMinorAxis);
 ...
 */
export function calculateTransferPath(transfer: any, systemCenter: { x:number, y:number }) : [number, number][] {
  if (!transfer) return [];
  const t = typeof transfer === 'string' ? JSON.parse(transfer) : transfer;

  const ellipse = {
    center: new Vec3(Number(t.centerx || 0) * KM_TO_M, Number(t.centery || 0) * KM_TO_M, 0),
  ...
  const start = new Vec3(Number(t.startpositionx || 0) * KM_TO_M, Number(t.startpositiony || 0) * KM_TO_M, 0);
  const end   = new Vec3(Number(t.targetpositionx || 0) * KM_TO_M, Number(t.targetpositiony || 0) * KM_TO_M, 0);
