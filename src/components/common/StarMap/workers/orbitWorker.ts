...
import type { MapPoint, PlanetData, StationData, WorkerFlightPlan } from '../types/mapTypes';
...
function _calculateWorldTimeSeconds(timeMs: number): number {
  ...
  const timeAccelerationFactor = (SIMULATION_INTERVAL / 86400) * PLANETARY_MOTION_FACTOR;
  return REFERENCE_EPOCH + (timeMs / 1000 - REFERENCE_EPOCH) * timeAccelerationFactor;
}

function keplerEquation(e: number, M: number): number {
  let Mnorm = ((M + Math.PI) % (2 * Math.PI) + 2 * Math.PI) % (2 * Math.PI) - Math.PI;
...
function orbitalPlaneToInertialInline(yOrb: number, xOrb: number, Ω: number, ω: number, i: number): [number, number, number] {
...
function preparePlanetCache(planets: PlanetData[], stations: StationData[], starSystem: MapPoint | null) {
...
function computePlanetPositionCached(i: number, worldTime: number) {
  ...
  const posX = (starSystemForOrbits ? starSystemForOrbits.x : 0) + inertial[0] / SCALE_FACTOR;
  const posY = (starSystemForOrbits ? starSystemForOrbits.y : 0) - inertial[1] / SCALE_FACTOR;
...
function computeStationPositionCached(i: number, worldTime: number) {
  ...
  const yOrb = r * Math.sin(nu);
  const inertial = orbitalPlaneToInertialInline(yOrb, xOrb, Ω, ω, inc);
  const posX = (starSystemForOrbits ? starSystemForOrbits.x : 0) + inertial[0] / SCALE_FACTOR_STATIONS;
  const posY = (starSystemForOrbits ? starSystemForOrbits.y : 0) - inertial[1] / SCALE_FACTOR_STATIONS;
  return { x: posX, y: posY, nu };
}
...
function regenerateTrails(worldTime: number) {
  ...
  for (let i = 0; i < cachedPlanets.length; i++) {
    ...
    for (let s = 0; s <= TRAIL_SEGMENTS; s++) {
      ...
      const yOrb = r * Math.sin(nu);
      const inertial = orbitalPlaneToInertialInline(yOrb, xOrb, Ω, ω, inc);
      const x = (starSystemForOrbits ? starSystemForOrbits.x : 0) + inertial[0] / SCALE_FACTOR;
      const y = (starSystemForOrbits ? starSystemForOrbits.y : 0) - inertial[1] / SCALE_FACTOR;
...
function getFlightStatus(ship: any, plan: any) {
  if (!plan || !plan.segments) return { isInterSystem: false, systemId: ship.addresssystemid };
  const now = Date.now();
  const activeSegment = plan.segments.find((s: any) => now >= s.departure && now < s.arrival);
  if (activeSegment) {
    if (activeSegment.origin_system_id !== activeSegment.destination_system_id) {
      return { isInterSystem: true, origin: activeSegment.origin_system_id, dest: activeSegment.destination_system_id };
    }
...
function calculateShipPositionGalaxy(ship: any, plan: any, currentTime: number, sysMap: Map<string, MapPoint>): [number, number, number] | null {
  try {
    const activeSegment = plan?.segments?.find((s: any) => currentTime >= s.departure && currentTime < s.arrival);
    if (activeSegment) {
...
function calculateShipPositionInSystem(ship: any, plan: any, currentTime: number, targetLookup: Map<string, { x: number; y: number }>, currentSystem: MapPoint | null): [number, number] | null {
  try {
    const activeSegment = plan?.segments?.find((s: any) => currentTime >= s.departure && currentTime < s.arrival);
    if (!activeSegment) {
      const targetId = ship.addressplanetid ?? ship.addressstationid;
      const target = targetLookup.get(targetId);
      return target ? [target.x, target.y] : (currentSystem ? [currentSystem.x, currentSystem.y] : null);
    }
    if (activeSegment.transferellipse && currentSystem) {
      const duration = activeSegment.arrival - activeSegment.departure;
      const progress = Math.min(1, Math.max(0, (currentTime - activeSegment.departure) / duration));
      try {
        const res = shipPositionOnEllipse(activeSegment.transferellipse, progress, currentSystem);
...
function ensurePair(buffers: ArrayBuffer[] | null, bytesNeeded: number): ArrayBuffer[] {
  if (!buffers) return [new ArrayBuffer(bytesNeeded), new ArrayBuffer(bytesNeeded)];
  const aValid = buffers[0] && buffers[0].byteLength >= bytesNeeded;
  const bValid = buffers[1] && buffers[1].byteLength >= bytesNeeded;
  if (!aValid || !bValid) return [new ArrayBuffer(bytesNeeded), new ArrayBuffer(bytesNeeded)];
  return buffers;
}

function ensureBuffers(planetCount: number, stationCount: number, shipCount: number) {
...
function safeFloat32View(buffers: ArrayBuffer[] | null, index: number, bytesNeeded: number): Float32Array {
  try {
...
function safeUint8View(buffers: ArrayBuffer[] | null, index: number, bytesNeeded: number): Uint8ClampedArray {
  try {
    if (!buffers) throw new Error('no-buffers');
    const ab = buffers[index];
    if (!ab || ab.byteLength < bytesNeeded) throw new Error('detached');
...
function calculateAndPostEverything() {
  ...
  const planetPosBuf = safeFloat32View(planetPosBuffers, bufIdx, planetCount * 2 * 4);
  const planetColorBuf = safeUint8View(planetColorBuffers, bufIdx, planetCount * 4);
  const stationPosBuf = safeFloat32View(stationPosBuffers, bufIdx, stationCount * 2 * 4);
  const stationColorBuf = safeUint8View(stationColorBuffers, bufIdx, stationCount * 4);
  ...
  for (let i = 0; i < planetCount; i++) {
    ...
    planetColorBuf[i * 4] = col[0]; planetColorBuf[i * 4 + 1] = col[1];
    planetColorBuf[i * 4 + 2] = col[2]; planetColorBuf[i * 4 + 3] = col[3];
  }
  ...
  for (let i = 0; i < stationCount; i++) {
    ...
    stationColorBuf[i * 4] = 0; stationColorBuf[i * 4 + 1] = 255;
    stationColorBuf[i * 4 + 2] = 0; stationColorBuf[i * 4 + 3] = 255;
  }
  ...
  if (!isGalaxyViewCached) {
    for (let i = 0; i < planetCount; i++) {
      targetLookup.set(cachedPlanets[i].raw.planetid, { x: planetPosBuf[i * 2], y: planetPosBuf[i * 2 + 1] });
    }
    for (let i = 0; i < stationCount; i++) {
      targetLookup.set(cachedStations[i].raw.stationid, { x: stationPosBuf[i * 2], y: stationPosBuf[i * 2 + 1] });
    }
  }

  for (let si = 0; si < shipCount; si++) {
    const ship = shipsForInterval[si];
    let x = 0, y = 0, bearing = ship.bearing || 0, visible = 1;
    ...
    if (isGalaxyViewCached) {
      if (flightStatus.isInterSystem) {
        ...
        if (pos) { x = pos[0]; y = pos[1]; bearing = pos[2]; }
        else visible = 0;
      } else {
        const sys = systemsMap.get(ship.addresssystemid) || systemsMap.get(flightStatus.systemId);
        if (sys) { x = sys.x; y = sys.y; }
        else visible = 0;
      }
    } else {
      if (flightStatus.isInterSystem) {
        visible = 0;
      } else if (flightStatus.systemId !== centeredSystemCached?.originalSystemId) {
        visible = 0;
      } else {
        const pos = calculateShipPositionInSystem(ship, plan, now, targetLookup, centeredSystemCached);
  ...
  const msg: any = {
    type: "tick-update",
    time: Date.now(),
    planetCount, stationCount, shipCount,
    ...
    workerPlans: (updatedPlans && updatedPlans.length > 0) ? updatedPlans : undefined,
    stats: { duration, avg }
  };
  ...
  try {
    ...
    delete msg.planetPos; delete msg.planetColor; delete msg.stationPos; delete msg.stationColor; delete msg.shipPos;
...
self.onmessage = function (e: MessageEvent) {
  ...
  if (type === "init-data") {
    try {
      preparePlanetCache(d.payload.planets || [], d.payload.stations || [], d.payload.centeredSystem || null);
      ...
      mainIntervalId = setInterval(calculateAndPostEverything, UPDATE_INTERVAL_MS) as unknown as number;
      calculateAndPostEverything();
  ...
  if (type === "update-ships") {
    ...
    plansMap = new Map((d.payload?.activeFlightPlans || []).map((p: any) => {
      if (p.departuretimestamp && !(p as any)._departureMs) (p as any)._departureMs = Date.parse(p.departuretimestamp);
      if (p.arrivaltimestamp && !(p as any)._arrivalMs) (p as any)._arrivalMs = Date.parse(p.arrivaltimestamp);
...
(self as any).addEventListener && (self as any).addEventListener("close", () => {
  if (mainIntervalId) clearInterval(mainIntervalId);
});
