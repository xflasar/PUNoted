...
import { AnimatedShipData, FlightPlan, WorkerFlightPlan } from '../types/mapTypes';
...
export const useShipWebSocket = ({
    ...
    const { rawInitialShips, setRawInitialShips, setAnimatedShipData, setActiveFlightPlans } = useGlobalData();
    ...
    useGlobalWs(mode === 'public' ? undefined : (mapChannel || undefined), (message) => {
    ...
    useEffect(() => {
        ...
        const isDataReady = systemsPoints?.length > 0 && 
                           Object.keys(allPlanetsData || {}).length > 0;
        if (!isDataReady) return;

        const labelGetter = getOriginDestinationLabel(systemsPoints, allPlanetsData, allStationsData);
        ...
        rawInitialShips.forEach((ship: any) => {
            ...
            if (ship.flight) {
                ...
                const destinationLabel = labelGetter(ship.flight as WorkerFlightPlan, false);
                enrichedPlan = { ...ship.flight, shipid: ship.shipid, origin: originLabel, destination: destinationLabel };
