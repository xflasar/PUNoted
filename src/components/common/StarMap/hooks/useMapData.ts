...
import type { MapPoint, Sector, PlanetData, StationData, GatewayData } from '../types/mapTypes';
import { assignEmpireColors } from '../utils/colors';
import {
  resolveInitialOverlapsAsync,
  buildSystemConnections,
} from '../utils/grid';
import { pointInPolygon, convexHull, densifyPolygon, chaikinSmooth, calculateCentroid } from '../utils/geometry';
...
export const useMapData = () => {
    ...
    // Connections
    const [systemConnections, setSystemConnections] = useState<{ sourcePosition: number[]; targetPosition: number[] }[]>([]);
    const [gatewayConnections, setGatewayConnections] = useState<{ sourcePosition: number[]; targetPosition: number[]; type: string }[]>([]); 
    ...
    const [allGatewaysData, setAllGatewaysData] = useState<Record<string, GatewayData[]>>({}); 

    const [maxSystemPopulation, setMaxSystemPopulation] = useState<number>(0);
    const [contentBounds, setContentBounds] = useState<{ minX: number; minY: number; maxX: number; maxY: number; } | null>(null);
    ...
    const fetchData = async () => {
      ...
      try {
        const res = await fetch(
          `https://api.punoted.net/dashboard_map`
        );
        ...
        for (const s of data.systems || []) {
          const key = s.systemid != null ? String(s.systemid) : `${s.positionx}:${s.positiony}`;
        ...
        for (const g of data.gateways || []) {
            ...
            if (g.system_id) {
                ...
                if (g.naturalid) gatewayIdToSystemId.set(String(g.id), String(g.system_id));
            }
        ...
        for (const [sysIdRaw, planets] of planetsBySystem.entries()) {
          ...
          for (const p of planets) {
            ...
            if (typeof rawBreakdown === "string") {
              try {
                ...
                console.error("Failed to parse planet population string:", rawBreakdown, e);
              }
            ...
            if (planetBreakdown) {
              for (const [type, count] of Object.entries(planetBreakdown)) {
                if (typeof count === "number" && count > 0) {
                  ...
                  systemPopTypeCounts[normalizedType] = (systemPopTypeCounts[normalizedType] || 0) + count;
                  totalPop += count;
                }
              }
            }
          ...
          const popBreakdownData = unsortedBreakdown.sort((a, b) => a.count - b.count);
          const counts: Record<string, number> = {};
          for (const p of planets) {
            const c = p.countrycode ?? "UN";
            counts[c] = (counts[c] || 0) + 1;
          }
          let majority = "UN", best = -1;
          ...
          const isCX = planets.some((p: any) => /(^|\s)CX(\s|$)/i.test(p.name || "")) || matched.isCX;
          ...
          matched.color = color;
          matched.isCX = isCX;
          matched.radiusHint = Math.max(5, matched.population && matched.population > 0 ? Math.log10(Math.max(1, matched.population)) * 3.5 + 5 : 5);
        ...
        const finalGatewayConnections: { sourcePosition: number[]; targetPosition: number[]; type: string }[] = [];
        ...
        for (const g of data.gateways || []) {
            ...
            if (g.outgoing_link_id) {
                const targetSysId = gatewayIdToSystemId.get(String(g.outgoing_link_id));
                if (targetSysId) {
            ...
            if (g.incoming_links && Array.isArray(g.incoming_links)) {
                for (const incomingGatewayId of g.incoming_links) {
                    const originSysId = gatewayIdToSystemId.get(String(incomingGatewayId));
                    if (originSysId) {
