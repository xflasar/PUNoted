...
import type { MapPoint, PlanetData, SystemStats, PlanetPosition, StationData, StationPosition } from '../types/mapTypes';
...
function _calculateWorldTime(timeSeconds: number, useTimeAccelerationFactor = true): number {
    const timeAccelerationFactor = useTimeAccelerationFactor ? (SIMULATION_INTERVAL / 86400) * PLANETARY_MOTION_FACTOR : 1;
    return REFERENCE_EPOCH + (timeSeconds - REFERENCE_EPOCH) * timeAccelerationFactor;
}

function matrixMultiply(m1: number[], m2: number[] | number[]): number[] | null {
    if (m2.length === 9) {
        return [
...
function orbitalPlaneToInertial(x: number[], Ω: number, ω: number, i: number): number[] {
...
function calculateMovingPlanetPosition(starSystem: MapPoint, planetData: PlanetData, currentTimeSeconds: number): any {
    const worldTime = _calculateWorldTime(currentTimeSeconds / 1000, true);
    const t0 = 0, M0 = 0;
    const a = planetData.semimajoraxis ?? 1e10, e = planetData.eccentricity ?? 0, i = planetData.inclination ?? 0, Ω = planetData.rightascension ?? 0, ω = planetData.periapsis ?? 0;
    let GM = 398600441800000;
    if (planetData.mass) GM = G * planetData.mass;
    if (starSystem.mass) GM = G * (planetData.mass + starSystem.mass);
    const p = a * (1 - e * e), n = Math.sqrt(GM / Math.pow(a, 3)), M = M0 + n * (worldTime - t0), E = keplerEquation(e, M);
    const nu = 2 * Math.atan(Math.sqrt((1 + e) / (1 - e)) * Math.tan(E / 2)), r = p / (1 + e * Math.cos(nu));
    const xOrb = r * Math.cos(nu), yOrb = r * Math.sin(nu);
    const inertial = orbitalPlaneToInertial([yOrb, xOrb, 0], Ω, ω, i);
    const posX = starSystem.x + inertial[0] / SCALE_FACTOR, posY = starSystem.y - inertial[1] / SCALE_FACTOR;
    return { x: posX, y: posY, planetid: planetData.planetid, name: planetData.planetname, parentSystemId: starSystem.originalSystemId ?? "", orbitalRadius: planetData.scaledOrbitalRadius ?? (a / SCALE_FACTOR), planetPopulation: planetData.planetPopulation ?? 0, scaledOrbitalRadius: planetData.scaledOrbitalRadius ?? (a / SCALE_FACTOR), scaledPlanetRadius: planetData.scaledPlanetRadius ?? 1, semimajoraxis: planetData.semimajoraxis, eccentricity: planetData.eccentricity, inclination: planetData.inclination, rightascension: planetData.rightascension, periapsis: planetData.periapsis, orbitindex: planetData.orbitindex };
}

function calculateMovingStationPosition(starSystem: MapPoint, stationData: StationData, currentTimeSeconds: number): any {
    const worldTime = _calculateWorldTime(currentTimeSeconds / 1000, true);
    const t0 = 0, M0 = 0;
    const a = stationData.semimajoraxis ?? 1e10, e = stationData.eccentricity ?? 0, i = stationData.inclination ?? 0, Ω = stationData.rightascension ?? 0, ω = stationData.periapsis ?? 0;
    ...
    const xOrb = r * Math.cos(nu), yOrb = r * Math.sin(nu);
    const inertial = orbitalPlaneToInertial([yOrb, xOrb, 0], Ω, ω, i);
    const posX = starSystem.x + inertial[0] / SCALE_FACTOR_STATIONS, posY = starSystem.y - inertial[1] / SCALE_FACTOR_STATIONS;
    return { x: posX, y: posY, stationid: stationData.stationid, name: stationData.name, parentSystemId: starSystem.originalSystemId ?? "", orbitalRadius: (a / SCALE_FACTOR_STATIONS), scaledOrbitalRadius: (a / SCALE_FACTOR_STATIONS), scaledStationRadius: 1, semimajoraxis: stationData.semimajoraxis, eccentricity: stationData.eccentricity, inclination: stationData.inclination, rightascension: stationData.rightascension, periapsis: stationData.periapsis };
}
...
export const useSystemViewSetup = (
    ...
    systemBoundsRef: React.MutableRefObject<{ minX: number; maxX: number; minY: number; maxY: number; } | null>,
    ...
    useEffect(() => {
        ...
        const maxA = Math.max(...planets.map((p) => Math.max(1, p.semimajoraxis * 1e10)), 1);
        const minA = planets.length > 0 ? Math.max(1, planets[0].semimajoraxis * 1e10) : 1;
        ...
        const maxPopPlanet = planets.reduce((max, current) => {
		...
		const maxPopLogFactor = Math.log10(Math.max(1, maxPopPlanet.planetPopulation!));
        ...
        const scaled = planets.map((p) => {
            ...
            const proportionalRadius = VISUAL_MIN_RADIUS + visualRange * exaggeratedNormalizedDistance;
			...
			const relativeSizeFactor = maxPopLogFactor > 0 ? planetPopLogFactor / maxPopLogFactor : 0;
        ...
        const stationsOrbits = stations.map((st) => calculateOrbitEllipse(centeredSystem as MapPoint, st as any, 128))
        ...
        if (initialPositions.length > 0) {
            ...
            const colorFactor = Math.min(1, Math.max(0, asteroidCount / MAX_ASTEROID_COUNT));
            ...
            const finalColor: [number, number, number, number] = [colorR, colorG, colorB, 150];
            const polygon = createCirclePolygon([centeredSystem.x, centeredSystem.y], totalRadius);
            ...
            const systemWidth = systemBoundsRef.current.maxX - systemBoundsRef.current.minX;
            const systemHeight = systemBoundsRef.current.maxY - systemBoundsRef.current.minY;
            ...
            setSystemBoundingBox(
                [{ polygon: polygon, color: finalColor }]
            )
    ...
    }, [centeredSystem, allPlanetsData, allStationsData, setSystemViewState, setCurrentViewMode, initialPlanetZoomRef, ignoreOnViewStateChangeRef, systemBoundsRef]);
	
    return { orbitLines, systemBoundingBox, systemStats, initialPlanets, initialStations };
}
