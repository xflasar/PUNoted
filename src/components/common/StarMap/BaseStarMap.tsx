...
import type { MapPoint, PlanetPosition, AnimatedShipData, FlightPlan, LocationFocusTarget } from "./types/mapTypes";
...
const MemoizedDeckGL = React.memo(
  ...
  (prevProps, nextProps) =>
    prevProps.viewState === nextProps.viewState && deepCompareLayers(prevProps.layers, nextProps.layers)
);
...
const BaseStarMap: React.FC<BaseStarMapProps> = ({ mode, overrideShips, focusTarget }) => {
  // Basic refs & theme
  const theme = useTheme();
  const mapRef = useRef<HTMLDivElement | null>(null);
  const deckRef = useRef<DeckGLRef<OrthographicView[]> | null>(null);
  const contentBounds = useRef<{ minX: number; minY: number; maxX: number; maxY: number } | null>(null);
  ...
  useEffect(() => {
    ...
    try {
      ...
      animationWorkerRef.current = new Worker(new URL("./workers/orbitWorker.ts", import.meta.url), {
        type: "module",
      });
    } catch (err) {
  ...
  useEffect(() => {
    ...
    if (!isInteracting) {
      try {
        animationWorkerRef.current.postMessage({ type: "resume-orbit-interval" });
      } catch (err) {
  ...
  const handleGroupVisibilityChange = useCallback((group: string) => {
    setVisibleCorpGroups((prev) => ({ ...prev, [group]: prev[group] === false ? true : false }));
  }, []);
  ...
  const handleToggleAllCorpVisibility = useCallback((allGroups: string[], visible: boolean) => {
  ...
  const debouncedSetViewState = useMemo(() => {
    return (typeof window !== "undefined" ? (fn: any) => fn : (fn: any) => fn) as any;
  }, []);
  ...
  useEffect(() => {
    ...
    if (!observerAvailable) {
      ...
      setMapSize({ w: Math.max(1, Math.round(rect.width)), h: Math.max(1, Math.round(rect.height)) });
      return;
    }
  ...
  const { onSystemClick, handleViewStateChange: navigationHandleViewStateChange } = useViewNavigation({
    systemsPoints,
    isGalaxyView,
    galaxyViewState,
    setGalaxyViewState,
  ...
  const handleViewStateChange = useCallback(
    (params: any) => {
      ...
      if (shallowViewEqual(viewStateRef.current, viewState)) {
        ...
        const isUserInteracting = interactionState?.isDragging || interactionState?.isZooming || interactionState?.isPanning;
      ...
      if (isUserInteracting) {
        ...
        const cleanState = { ...viewState, transitionDuration: 0, transitionInterpolator: null, transitionEasing: null };
        if (currentViewMode === "galaxy") cleanState.zoom = Math.min(cleanState.zoom, 1.99);
  ...
  const { ownShips, corpShipsGrouped, visibleAnimatedShipData } = useMemo(() => {
    ...
    for (const ship of effectiveShips) {
      const isMine = ship.is_owner_ship || (currentUserId && String(ship.ownerId) === String(currentUserId));
    ...
    return { ownShips: own, corpShipsGrouped: finalCorpGroups, visibleAnimatedShipData: allVisible };
  ...
  const effectiveSetFlightPlans = mode === "shipping" ? (() => {}) : setGlobalActiveFlightPlans;

  // ship websocket hookup (passes worker ref; the hook should guard usage)
  useShipWebSocket({ mode, systemsPoints, allPlanetsData, allStationsData, animationWorker: animationWorkerRef.current });
  ...
  const handleDeckHover = useCallback((info: any) => {
    ...
    if (info.object) {
      if (Array.isArray(info.object.ships)) {
        const shipNames = info.object.ships.map((s: any) => s.name || s.id).join(", ");
    ...
    for (const system of systemsPoints) {
      ...
      const foundSector = sectors.find((sector) => {
        try {
          ...
          return (sector && sector.vertices) ? (Array.isArray(sector.vertices) && sector.vertices.length > 0 && (require("./utils/geometry").pointInPolygon ? (false as any) : false)) : false;
        } catch {
          return false;
        }
      });
  ...
  const liveSelectedPlanet = useMemo(() => {
    ...
    const moving = activePlanets?.find((p: any) => p.planetid === selectedPlanet.planetid);
      ...
      if (focusTarget.type === "SYSTEM") {
        ...
        if (ship) {
          if (ship.addresssystemid && !ship.plan) {
            const sys = systemsPoints?.find((s: any) => s.originalSystemId === ship.addresssystemid);
            if (sys && centeredSystem?.originalSystemId !== sys.originalSystemId) onSystemClick(sys);
          } else {
  ...
  }, [focusTarget, systemsPoints, effectiveShips, centeredSystem, onSystemClick, handleShipSelect]);

  // search handlers
  const handleSearch = useCallback((query: string) => setTriggeredSearchQuery(query), []);
  const handleCloseSearchResults = useCallback(() => setTriggeredSearchQuery(""), []);
