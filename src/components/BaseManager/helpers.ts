import { EXPERT_BONUS, FACTION_BONUSES, LUXURY_MULTIPLIERS, WORKER_NEEDS, PERMIT_AREAS } from './constants';
 ...
 */
export const formatCurrency = (val: number): string => new Intl.NumberFormat('en-US', { notation: 'compact', maximumFractionDigits: 1 }).format(val);
 ...
 */
const normalizeWorker = (w: string): string => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase();
...
export const calculateBaseMetrics = (params: EngineParams) => {
    const { 
        activeData, activeBuildings, activeRecipes, activeNeeds, 
        experts, activeCogc, planetFactor, getPrice,
        faction = "no faction", usedPermits = 1, totalPermits = 1 
    } = params;

    const permitArea = PERMIT_AREAS[Number(activeData.permitLevel)] || 500;
    let usedArea = 25; 
    let storageMaxWeight = 1500, storageMaxVolume = 1500, totalCapEx = 0; // Default 1500/1500
    ...
    const addBuildReq = (ticker: string, count: number) => {
        ...
        if (bInfo?.buildReq) bInfo.buildReq.forEach((req: any) => {
            buildMaterials[req.ticker] = (buildMaterials[req.ticker] || 0) + (Number(req.amount) * count);
        });
    };
    ...
    activeData.platforms.forEach((p: any) => {
        ...
        if (b) {
            ...
            if (b.workers) Object.keys(b.workers).forEach(w => workforce[normalizeWorker(w)].demand += Number((b.workers as any)[w]) * amount);
        }
    });
    ...
    activeData.infrastructure.forEach((i: any) => {
        ...
        if (b) {
            usedArea += Number(b.area) * amount;
            addBuildReq(b.ticker, amount);
            if (b.supply) Object.keys(b.supply).forEach(w => workforce[normalizeWorker(w)].supply += Number((b.supply as any)[w]) * amount);
    ...
    activeData.platforms.forEach((p: any) => {
        ...
        const activeCogcLower = activeCogc ? String(activeCogc).toLowerCase().trim() : "";

        const expCategoryKey = Object.keys(experts).find(k => String(k).toLowerCase().trim() === bCategoryLower);
        const expCount = expCategoryKey ? experts[expCategoryKey] : 0;
        const expertBonus = EXPERT_BONUS[expCount] || 0;
        
        const cogcBonus = (activeCogcLower === bCategoryLower && activeCogcLower !== "") ? 1.25 : 1.0;
        
        const baseFactionBonus = FACTION_BONUSES[activeFactionLower]?.[bCategoryLower] || 0;
        const factionBonus = baseFactionBonus * permitModifier;

        const resourceFactor = (Number(planetFactor) || 100) / 100;

        const totalEfficiency = (buildingWorkforceEff / 100) * (1 + expertBonus) * cogcBonus * (1 + factionBonus) * resourceFactor;
        ...
        const bRecipes = (p.activeRecipes || []).map((id: string) => {
            const baseRecipe = activeRecipes.find(r => r.id === id);
            if (!baseRecipe) return null;
            return {
                ...baseRecipe,
                duration: Number(baseRecipe.duration) / Math.max(0.0001, totalEfficiency)
            };
        }).filter(Boolean);
        ...
        if (bRecipes.length > 0 && totalEfficiency > 0) {
            ...
            bRecipes.forEach((r: any) => totalQueueDurationActual += (Number(r.duration) || 1));
    ...
    let totalDailyProfit = 0, totalWeightImport = 0, totalWeightExport = 0, totalVolumeImport = 0, totalVolumeExport = 0;
    ...
    return {
        usedArea, permitArea, storageMaxWeight, storageMaxVolume, totalCapEx,
        workforce, materialIO, platformEfficiencies, platformBuildingQueues, buildMaterials,
        totalWeightImport, totalWeightExport, totalVolumeImport, totalVolumeExport, totalDailyProfit,
        allUniqueMaterials, platformActiveRecipes
    };
};
...
export const aggregateNodeProduction = (
    ...
    staticData.materials?.forEach((m: any) => {
        m.inputRecipes?.forEach((r: any) => {
            const actualOutputs = r.outputs?.length > 0 ? r.outputs : [{ ticker: m.ticker, amount: 1 }];
            activeRecipes.push({
                id: r.processid || r.id,
    ...
    users.forEach(user => {
        if (!user.baseData || user.baseData.status === 'uninitialized' || !user.baseData.platforms) {
            result.userProd.push({ username: user.displayName || user.username, isRegistered: user.isRegistered, prod: 0, cons: 0, net: 0 });
            return;
        }
        ...
        user.baseData.platforms.forEach((p: any) => {
            ...
            const targetRecipes = recipesOnPlatform.filter((r: any) => 
                r.outputs?.some((out: any) => String(out.ticker).toUpperCase() === result.targetTicker)
            );
            ...
            targetRecipes.forEach((recipe: any) => {
                ...
                const loopsPerDay = (86400 / (Number(recipe.duration) || 1)) * buildingsPerRecipe;
                ...
                recipe.inputs?.forEach((input: any) => {
                    ...
                    if (String(input.ticker).toUpperCase() === result.targetTicker) {
                        ...
                        result.aggregatedInputs[input.ticker] = (result.aggregatedInputs[input.ticker] || 0) + consumedDaily;
                    }
                });
                ...
                recipe.outputs?.forEach((output: any) => {
                    if (String(output.ticker).toUpperCase() !== result.targetTicker) {
                        const outputAmt = Number(output.amount) || 0;
                        result.byproducts[output.ticker] = (result.byproducts[output.ticker] || 0) + (outputAmt * loopsPerDay * resourceFactor);
                    }
                });
            });
        });
