...
export const CommoditySidebar: React.FC<CommoditySidebarProps> = React.memo(
	({
		marketData,
		selectedTicker,
		selectedExchange,
		...
		const filteredCommodities = useMemo(() => {
			...
			const terms = searchQuery
				...
				.filter(Boolean);
			if (terms.length === 0) return commodities;

			return commodities.filter((c) => {
				const tickerLower = c.ticker.toLowerCase();
				return terms.some((term) => {
					if (exactMatch) {
						return tickerLower === term;
					}
					return tickerLower.includes(term);
				});
			});
		}, [commodities, searchQuery, exactMatch]);
