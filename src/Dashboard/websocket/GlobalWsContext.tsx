...
export const GlobalWsProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    try {
      ...
      if (tokenParts.length === 3) {
        ...
        if (payload.exp && payload.exp < now) {
          console.warn("WS: Token expired locally. Redirecting to landing page...");
    ...
    ws.onmessage = (event) => {
      ...
      try {
        ...
        if (data.error && data.error.toLowerCase().includes('expire')) {
          console.warn("WS: Server reported token expiry via message. Redirecting...");
          localStorage.removeItem('authToken');
    ...
    ws.onclose = (event) => {
      if (wsRef.current === ws) {
        ...
        if (isAuthError) {
          console.warn("WS: Server rejected auth via close event. Redirecting...");
          localStorage.removeItem('authToken');
          intentionalClose.current = true;
          window.location.href = '/';
          return;
    ...
    return (
        <GlobalWsContext.Provider value={{ 
            status, sendJson, subscribe, unsubscribe, addMessageListener, removeMessageListener 
        }}>
            {children}
