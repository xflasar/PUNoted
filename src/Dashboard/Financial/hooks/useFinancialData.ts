...
import { FinancialPayload, PartnerMetrics, Transaction } from '../types/finance';
...
export const useFinancialData = () => {
  ...
  const fetchFinances = async () => {
    ...
    try {
      ...
      const response = await fetch('https://api.punoted.net/dev/internal/finances/overview', {
      ...
      if (savedCurrency && jsonData.Currencies) {
        const foundIndex = jsonData.Currencies.findIndex(c => c.Currency === savedCurrency);
  ...
  const handleTabChange = (event: React.SyntheticEvent, newValue: number) => {
    ...
    if (data && data.Currencies[newValue]) {
      localStorage.setItem('preferredCurrency', data.Currencies[newValue].Currency);
    }
  };
  ...
  const netPending = currentData ? currentData.PendingReceivable - currentData.PendingPayable : 0;
  
  const incomeExpense30D = currentData?.CashFlows.map(flow => ({
    name: flow.Category.replace('_', ' '),
    Income: flow['30D'].Income,
    Expense: -flow['30D'].Expense, 
  })) || [];

  const pieChartData = currentData?.CashFlows
    .map(flow => ({ name: flow.Category.replace('_', ' '), value: flow['30D'].Income }))
  ...
  const topPartners = useMemo(() => {
    ...
    currentData.Transactions.forEach(tx => {
      ...
      const existing = partnerMap.get(tx.PartnerCode) || { code: tx.PartnerCode, name: tx.PartnerName, volume: 0, net: 0 };
      existing.volume += Math.abs(tx.Amount);
      existing.net += tx.Amount;
      partnerMap.set(tx.PartnerCode, existing);
    });

    return Array.from(partnerMap.values()).sort((a, b) => b.volume - a.volume).slice(0, 15); 
  }, [currentData]);

  return {
    data, loading, error, activeCurrencyIndex, handleTabChange, fetchFinances,
    currentData, netPending, incomeExpense30D, pieChartData, topPartners
  };
};