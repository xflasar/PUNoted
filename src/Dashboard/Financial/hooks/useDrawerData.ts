...
import { Transaction, TransactionDetail, PublicCompanyProfile } from '../types/finance';

export const useDrawerData = (selectedTx: Transaction | null, selectedPartnerCode: string | null, transactions: Transaction[]) => {
  ...
  const [companyProfile, setCompanyProfile] = useState<PublicCompanyProfile | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchTxDetails = async () => {
      ...
      try {
        ...
        const response = await fetch(`https://api.punoted.net/dev/internal/finances/transaction/${selectedTx.Id}`, {
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      ...
      try {
        const response = await fetch(`https://api.punoted.net/dev/v1/company/${selectedPartnerCode}`);
  ...
  return { txDetails, loadingTxDetails, companyProfile, loadingProfile, selectedPartnerStats };
};