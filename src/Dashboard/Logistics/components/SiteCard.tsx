...
const SiteCard: React.FC<SiteCardProps> = ({ site, onSelect }) => {
	...
	const getSiteStatus = (site: Site) => {
		const netTonnageChange =
			site.dailyProduction.reduce((acc, cur) => acc + cur.tonnage, 0) -
			site.dailyConsumption.reduce((acc, cur) => acc + cur.tonnage, 0);
		...
		>
			...
			>
				<Box sx={{ display: "flex", alignItems: "center" }}>
					<MapPin
						size={20}
						style={{ marginRight: 8, color: theme.palette.primary.light }}
					/>
			...
			<Box sx={{ display: "flex", gap: 1 }}>
				<Tooltip
					title={`Tonnage: ${Math.round((site.siteStorage.currentTonnage / site.siteStorage.maxTonnage) * 100)}%`}
				>
					<Box
						sx={{
							flex: 1,
					...
					>
						<LinearProgress
							variant="determinate"
							value={
								(site.siteStorage.currentTonnage /
									site.siteStorage.maxTonnage) *
								100
							}
							sx={{
								position: "absolute",
						...
						<Box sx={{ position: "relative", zIndex: 1 }}>
							<Typography variant="caption" component="p">
								<strong>
									{Math.round(site.siteStorage.currentTonnage).toLocaleString()}{" "}
									/ {Math.round(site.siteStorage.maxTonnage).toLocaleString()} t
								</strong>
							</Typography>
						</Box>
					</Box>
				...
				<Tooltip
					title={`Volume: ${Math.round((site.siteStorage.currentVolume / site.siteStorage.maxVolume) * 100)}%`}
				>
					<Box
						sx={{
							flex: 1,
					...
					>
						<LinearProgress
							variant="determinate"
							value={
								(site.siteStorage.currentVolume / site.siteStorage.maxVolume) *
								100
							}
							sx={{
								position: "absolute",
						...
						<Box sx={{ position: "relative", zIndex: 1 }}>
							<Typography variant="caption" component="p">
								<strong>
									{Math.round(site.siteStorage.currentVolume).toLocaleString()}{" "}
									/ {Math.round(site.siteStorage.maxVolume).toLocaleString()} m³
								</strong>
							</Typography>
						</Box>
					</Box>
