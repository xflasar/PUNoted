...
import { Box, CircularProgress, Alert, ToggleButtonGroup, ToggleButton, Typography, Paper } from '@mui/material';
import SitesList from './SitesList';
import ShipsList from './ShipsList';
import AutomatedLogistics from './AutomatedLogistics';
import type { Site, Ship, LogisticsSummary, TransportRecommendation, CX } from './types';
...
const Logistics: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            setError(null);
            try {
                const endpoints = ['sites', 'ships', 'cxs', 'summary', 'recommendations'];
                ...
                for (const res of responses) {
                    if (!res.ok) {
                        ...
                        console.error(`Failed to fetch data from ${res.url}: ${res.statusText}`, errorBody);
                        throw new Error(`Failed to fetch data: ${res.statusText}`);
                    }
                }

                const [sitesData, shipsData, cxsData, summaryData, recommendationsData] = await Promise.all(responses.map(res => res.json()));
                ...
                console.error(err);
                setError(`Could not load logistics data. Please ensure the backend service is running. [${err.message}]`);
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleViewChange = (event: React.MouseEvent<HTMLElement>, newView: 'automated' | 'manual' | null) => {
        if (newView !== null) {
            setViewMode(newView);
        }
    };
    ...
    const handleAssignShip = async (shipId: string, siteId: string | null) => {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/logistics/ships/${shipId}/assign`, {
                method: 'POST',
                headers: { 
    ...
    if (loading) {
        return <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '80vh' }}><CircularProgress /><Typography variant="h6" sx={{ ml: 2 }}>Loading Logistics Data...</Typography></Box>;
    }
    if (error) {
            ...
            <Box sx={{ flexGrow: 1, overflow: 'hidden' }}>
                {viewMode === 'automated' ? (
                    ...
                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2, height: '100%' }}>
                        {/* Column 1: Sites */}
                        <Box sx={{ flex: '1 1 500px', minWidth: 350, display: 'flex', flexDirection: 'column', minHeight: 0, maxHeight: '100%', overflow: 'hidden' }}>
                             <Typography variant="h5" gutterBottom>Site Operations</Typography>
                             <Paper sx={{ flexGrow: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column', background: 'transparent' }}>
                        ...
                        {/* Column 2: Ships */}
                        <Box sx={{ flex: '1 1 500px', minWidth: 350, display: 'flex', flexDirection: 'column', minHeight: 0, maxHeight: '100%', overflow: 'hidden' }}>
                            <Typography variant="h5" gutterBottom>Fleet Management</Typography>
                            <Paper sx={{ flexGrow: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column', background: 'transparent'}}>
                                <ShipsList ships={ships} sites={[...sites, ...cxs]} onAssignShip={handleAssignShip} />
                            </Paper>
                        </Box>
                    </Box>
                )}
