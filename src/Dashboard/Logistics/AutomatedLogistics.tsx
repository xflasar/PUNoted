...
import { Grid, Box, Typography, Paper, Accordion, AccordionSummary, AccordionDetails, Chip, useTheme } from '@mui/material';
...
const priorityOrder: TransportRecommendation['priority'][] = ['critical', 'high', 'medium', 'low'];
...
const AutomatedLogistics: React.FC<AutomatedLogisticsProps> = ({ recommendations, onApprove, onReject }) => {
    ...
    return (
        <Box>
            ...
            {priorityOrder.map(priority => {
                ...
                return (
                    <Accordion key={priority} defaultExpanded={priority === 'critical' || priority === 'high'} sx={{
                        ...
                        >
                            <Chip label={meta.label} color={meta.color as any} icon={meta.icon} />
                            <Typography variant="subtitle1">{recs.length} recommendation(s)</Typography>
                        </AccordionSummary>
                        <AccordionDetails sx={{ backgroundColor: theme.palette.background.paper, p: 2 }}>
