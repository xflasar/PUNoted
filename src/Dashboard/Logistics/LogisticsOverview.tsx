...
import { Paper, Box, Typography, Grid, Divider, Chip, Tooltip } from '@mui/material';
import { LogisticsSummary } from './types';
...
const LogisticsOverview: React.FC<LogisticsOverviewProps> = ({ summary }) => {
    ...
    const isInsufficient = transportAnalysis?.sufficiencyStatus === 'insufficient';
            ...
            <Grid container spacing={2}>
                <Grid item xs={12} md={6}>
                    <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                        ...
                        <Typography variant="h6" component="h3" sx={{ ml: 1 }}>Active Bottlenecks</Typography>
                    </Box>
                    <Divider />
                                ...
                                {bottlenecks.map((b, index) => (
                                    <Tooltip key={index} title={b.details}>
                                        <Chip
                                            ...
                                            color={b.type === 'material_shortage' ? 'error' : 'warning'}
                                        />
                                    </Tooltip>
                                ))}
                            </Box>
                        ) : (
                            <Typography variant="body1" color="text.secondary" sx={{pt: 2, textAlign: 'center'}}>
                ...
                <Grid item xs={12} md={6}>
                    <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                        {isInsufficient ? <Warning color="error" /> : <CheckCircle color="success" />}
                        <Typography variant="h6" component="h3" sx={{ ml: 1 }}>Transport Status (Daily)</Typography>
                    </Box>
                    <Divider />
                    <Box sx={{ mt: 1, maxHeight: 150, overflowY: 'auto' }}>
                        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, pt: 1 }}>
                            <Chip size="small" variant="outlined" label={`Tonnage Capacity: ${transportAnalysis.totalShipCapacityTonnage.toLocaleString()} t`} />
                            <Chip size="small" variant="outlined" label={`Volume Capacity: ${transportAnalysis.totalShipCapacityVolume.toLocaleString()} m³`} />
                            <Chip size="small" variant="outlined" label={`Tonnage Required: ${transportAnalysis.requiredTransportTonnage.toLocaleString()} t`} />
                            <Chip size="small" variant="outlined" label={`Volume Required: ${transportAnalysis.requiredTransportVolume.toLocaleString()} m³`} />
                            <Chip
                                size="small"
                                variant="filled"
                                label={
