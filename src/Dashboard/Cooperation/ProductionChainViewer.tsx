...
import { Add as Plus, BugReport, DashboardCustomize } from "@mui/icons-material";
...
>(
	(
		...
		const [sourceRFNode, _setSourceRFNode] = useState<Node<ChainNodeData> | null>(null);
		...
		const handleNodeChanges: OnNodesChange = useCallback(
			(changes) => {
				...
				const dragEndChange = changes.some((c) => c.type === "position" && c.dragging === false);
			...
			[nodes, edges, canEdit, setNodes, getNewChainState, onChainUpdate, onSendSyncMessage]
		);
		...
		const handleEdgeChanges: OnEdgesChange = useCallback(
			...
			[nodes, edges, canEdit, setEdges, getNewChainState, onChainUpdate, onSendSyncMessage]
		);
		...
		const isValidConnection = useCallback(
			(connection: Connection): boolean => {
                ...
                if (sourceChainNodeData.siteName.includes("Mock") || targetChainNodeData.siteName.includes("Mock")) return true;

				return validateConnection(sourceChainNodeData, targetChainNodeData, materials);
			},
			[nodes, materials]
		);
		...
		const onConnect = useCallback(
			...
			[edges, nodes, canEdit, getNewChainState, onChainUpdate, onSendSyncMessage, setEdges]
		);
		...
		const onDrop = useCallback(
			...
			[nodes, edges, canEdit, getNewChainState, onChainUpdate, onSendSyncMessage, setNodes]
		);
		...
		const onConnectStart = useCallback(
			(_: any, { nodeId }: { nodeId: string | null }) => {
				...
				const allTargetNodes = initialChain?.nodes?.filter((n) => n.nodeId !== nodeId);
				const validIds = new Set<string>();

				allTargetNodes?.forEach((targetNodeData) => {
                    if (targetNodeData.siteName.includes("Mock") || sourceChainNodeData.siteName.includes("Mock")) {
                        validIds.add(targetNodeData.nodeId);
                    } else {
                        const isForwardValid = validateConnection(sourceChainNodeData, targetNodeData, materials);
                        const isReverseValid = validateConnection(targetNodeData, sourceChainNodeData, materials);
				...
				setValidationData({ sourceNodeId: nodeId, validTargetNodeIds: validIds });
			},
		...
		const onMoveEnd = useCallback(
			(_: any, viewport: React.SetStateAction<Viewport>) => { setLocalViewport(viewport); }, []
		);

		const handleMouseMove = useMouseSync(onSendSyncMessage, localStorage.getItem("displayName") || "", viewport);

		interface LockedNodeState { nodeId: string; userId: string; }
		
        const isNodeLockedByOther = useCallback(
			(nodeId: string, lockedNode: LockedNodeState[] | null, currentUserId: string): boolean => {
				if (!lockedNode) return false;
				return lockedNode.some((lock) => lock.nodeId === nodeId && lock.userId !== currentUserId);
			},
			[]
		);
		...
		const handleNodeDragStart: NodeDragHandler = useCallback(
			(event, node: Node, _nodes: Node[]) => {
				...
				onSendSyncMessage({ type: "NODE_LOCKED", payload: { nodeId: node.id, userId: userId } });
			},
		...
		const handleNodeDragStop: NodeDragHandler = useCallback(
			(_event, node: Node, currentNodes: Node[]) => {
				onSendSyncMessage({ type: "NODE_UNLOCK", payload: { nodeId: node.id, userId: userId } });
                ...
                setNodes((prevNodes) => {
                    ...
                    if (targetGroup) {
                        ...
                        let groupW = targetGroup.width || parseInt(targetGroup.style?.width as string) || 600;
                        let groupH = targetGroup.height || parseInt(targetGroup.style?.height as string) || 400;
			...
			[onSendSyncMessage, userId, setNodes, edges, getNewChainState, onChainUpdate]
		);
        ...
        const computeFlows = (nodes: Node[], edges: Edge[]) => {
            ...
            nodes.forEach(node => {
                ...
                const agg = aggregateNodeProduction(node.data?.assignedUsers || [], node.data?.materialTicker || "UNK", staticData, getPrice);
            ...
            Object.entries(edgesBySource).forEach(([sourceId, sourceEdges]) => {
                ...
                let totalDemand = 0;
                const targetDemands: { targetId: string, demand: number, edgeId: string }[] = [];
                ...
                sourceEdges.forEach(edge => {
                    ...
                    totalDemand += demand;
                    targetDemands.push({ targetId: targetNode.id, demand, edgeId: edge.id });
                });
		...
		return (
			<ConnectionToolContext.Provider value={connectionToolContextValue}>
				<Box className="production-chain-viewer" sx={{ width: "100%", height: "100%", position: "relative" }} onMouseMove={handleMouseMove}>
					<ReactFlow
						nodes={enrichedNodes}
						...
						fitView
					>
						<Controls showZoom={true} showFitView={true} showInteractive={canEdit} />
						<Background style={{ background: theme.palette.background.default }} gap={12} size={1} />
						<LiveCursorRenderer remoteCursors={remoteCursors} />
					</ReactFlow>

					<Box sx={{ position: "absolute", top: 10, right: 10, zIndex: 10, display: 'flex', gap: 1 }}>
                        <Tooltip title="Spawn Full Test Chain"><Button variant="outlined" onClick={handleLoadTestChain} startIcon={<BugReport />} color="warning" sx={{ bgcolor: 'background.paper' }}>Load Chain Test</Button></Tooltip>
                        <Tooltip title="Add Location Boundary Box"><Button variant="outlined" onClick={handleAddGroupArea} startIcon={<DashboardCustomize />} color="info" sx={{ bgcolor: 'background.paper' }}>Add Group Area</Button></Tooltip>
						<Tooltip title="Add New Node"><Button variant="contained" onClick={onAddNodeClick} startIcon={<Plus />} color="secondary">Add Node</Button></Tooltip>
					</Box>
				</Box>
