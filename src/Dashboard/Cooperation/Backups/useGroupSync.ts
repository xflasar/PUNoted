...
export const useGroupSync = ({
	...
	const cursorTimeoutsRef = useRef<Map<string, ReturnType<typeof setTimeout>>>(
		new Map(),
	);
	...
	const applyNodeRemove = useCallback(
	...
	const applyNodeUpdate = useCallback(
		(payload: NodeUpdatePayload, type: string) => {
			const latestChain = currentChainRef.current;
			if (!latestChain) return;
			...
			const existingIndex = latestChain.nodes!.findIndex(
				(n) => n.nodeId === nodeId,
			);
	...
	const applyEdgeUpdate = useCallback(
		(payload: any) => {
			...
			if (payload["type"] === "add") {
				...
				newLinks = safeCurrentLinks.filter(
					(link) => `${link.source}-${link.target}` !== payload["id"],
				);
			}
	...
	const sendMessage = useCallback(
		(message: Omit<SyncMessage, "userId">) => {
			...
			if (ws && ws.readyState === WebSocket.OPEN) {
				...
				const stateName = ws
					...
					: "NULL";
				console.warn(
					`WS: Message not sent (State: ${stateName}). Waiting for connection.`,
				);
			}
		},
		[userId],
	...
	useEffect(() => {
		setIsConnected(false);
		if (reconnectAttemptCount === 0) setHasEverBeenConnected(false);
		if (!groupId) {
			...
			try {
				...
				switch (message.type) {
					...
					case "NODE_LOCK_DENIED": {
						console.warn(
							`Lock for ${message.payload.nodeId} denied by server. Currently held by user ${message.payload.lockedByUserId}.                    `,
						);
						break;
					}
					...
					case "CURSOR_MOVE": {
						...
						const existingTimeout = cursorTimeoutsRef.current.get(
							message.userId,
						);
						if (existingTimeout) {
