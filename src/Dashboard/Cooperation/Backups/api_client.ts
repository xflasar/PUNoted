 ...
 */
export const changeUserRole = async (groupId: string, memberId: string, newRole: 'editor' | 'viewer') => {
    
    const response = await fetch(`https://api.punoted.net/groups/${groupId}/members/${memberId}/role`, {
        method: 'PUT',
        headers: { 
...
export const saveChainData = async (
    ...
    const response = await fetch(`${API_BASE_URL}/${groupId}/save_chain`, { // Updated endpoint
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
