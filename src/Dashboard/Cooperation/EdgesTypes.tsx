...
const TransshipmentEdge: React.FC<EdgeProps<any>> = ({
	...
	const shippingMetrics = useMemo(() => {
		...
		const materialTicker =
			sourceNode.data?.materialTicker || data?.materialTicker || "UNK";
		const material = materials?.find((m) => m.ticker === materialTicker) || {
			weight: 0,
			volume: 0,
		};

		const isCrossPlanet =
			sourcePlanetId && targetPlanetId && sourcePlanetId !== targetPlanetId;
