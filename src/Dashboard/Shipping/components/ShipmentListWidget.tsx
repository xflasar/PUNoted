...
const getDeadlineInfo = (contract: ShipmentContract) => {
	const deadline = contract.deadline
		...
		: addDays(new Date(contract.created_at), 7);
	const now = new Date();
		...
		<Tooltip title={`${item.type}`}>
			...
			>
				{isDone ? (
					...
					<Inventory sx={{ fontSize: 12, color, opacity: 0.7 }} />
				)}
				<Typography
					variant="caption"
			...
			>
				...
				>
					...
					<Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
						{(isUrgent || isLate) && (
							<Warning sx={{ fontSize: 14, color: timeColor }} />
						)}
						<Typography
							variant="caption"
							sx={{
								color: timeColor,
								fontWeight: isUrgent || isLate ? 800 : 500,
								fontSize: "0.7rem",
							}}
						>
							{deadlineText}
...
// --- MAIN WIDGET ---
export const ShipmentListWidget: React.FC<Props> = ({
	contracts,
	onSelect,
}) => {
	const theme = useTheme();
	...
	// --- 4. Renderers ---
	const renderOutboundRow = useCallback(
		(index: number, c: ShipmentContract) => (
			<Box sx={{ px: 1, pt: 0.5 }}>
				<ContractRow
					contract={c}
					accentColor={outboundColor}
	...
	const renderInboundRow = useCallback(
		(index: number, c: ShipmentContract) => (
			<Box sx={{ px: 1, pt: 0.5 }}>
				<ContractRow
					contract={c}
					accentColor={inboundColor}
					onClick={() => onSelect(c.contract_id)}
				/>
			</Box>
		),
	...
	return (
		...
		>
			...
			>
				...
				>
					<Search
						sx={{ fontSize: 16, color: theme.palette.text.disabled, mr: 1 }}
					/>
					<InputBase
			...
			>
				...
				>
					...
					>
						...
						{outboundExpanded ? (
							<ExpandLess sx={{ color: outboundColor }} />
						) : (
							<ExpandMore sx={{ color: outboundColor }} />
						)}
					</ListItem>
					...
					{outboundExpanded && (
						...
						>
							{outbound.length > 0 ? (
								<Virtuoso
									data={outbound}
									itemContent={renderOutboundRow}
									style={{ height: "100%" }}
								/>
							) : (
								<Box sx={{ p: 3, textAlign: "center", opacity: 0.5 }}>
									<Typography variant="caption">
										No active outbound contracts
									</Typography>
								</Box>
							)}
						</Box>
					...
					>
						...
						>
							...
							<ListItemText
								...
								secondaryTypographyProps={{
									variant: "caption",
									color: alpha(inboundColor, 0.7),
								}}
								sx={{ my: 0 }}
							/>
						</Stack>
						{inboundExpanded ? (
							<ExpandLess sx={{ color: inboundColor }} />
						) : (
							<ExpandMore sx={{ color: inboundColor }} />
						)}
					</ListItem>
					...
					{inboundExpanded && (
						...
						>
							{inbound.length > 0 ? (
								<Virtuoso
									data={inbound}
									itemContent={renderInboundRow}
									style={{ height: "100%" }}
								/>
							) : (
								<Box sx={{ p: 3, textAlign: "center", opacity: 0.5 }}>
									<Typography variant="caption">
										No active inbound contracts
									</Typography>
								</Box>
							)}
						</Box>
