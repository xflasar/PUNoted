...
/**
 * A card component that displays a summary of production items grouped by category.
 * Handles both standard categories and custom "drilldown" views for specific items.
 */
				...
				>
					<Box
						sx={{
							...
							borderBottom: `1px solid ${alpha(theme.palette[color].main, 0.2)}`,
							display: "flex",
							flexDirection: "row",
							...
							{subItems.length > 0 ? (
								subItems.map((sub, idx) => {
									const memberInfo = members?.find(
										(m) =>
											m.companyName === sub.player ||
											m.companyCode === sub.player,
									);
								...
								<TableRow>
									<TableCell
										colSpan={3}
										align="center"
										sx={{ fontStyle: "italic", color: "text.disabled", py: 2 }}
									>
