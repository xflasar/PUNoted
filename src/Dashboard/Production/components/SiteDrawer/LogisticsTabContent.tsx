...
export const LogisticsTabContent: React.FC<any> = ({
	...
	const activeSelected = viewMode === "dropoff" ? selectedMaterials : selectedSurplusMaterials;
	const activeToggleAll = viewMode === "dropoff" ? toggleAllLogistics : toggleAllSurplus;
	const activeToggleMaterial = viewMode === "dropoff" ? toggleMaterial : toggleSurplusMaterial;
			...
			<Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 1 }}>
				<Box sx={{ display: "flex", gap: 1 }}>
					<Button
						...
						sx={{ height: 24, fontSize: "0.7rem", py: 0, px: 1, borderColor: viewMode === "dropoff" ? "transparent" : alpha(theme.palette.text.primary, 0.2) }}
					>
						Drop-off (Inputs)
					</Button>
					<Button
						...
						sx={{ height: 24, fontSize: "0.7rem", py: 0, px: 1, borderColor: viewMode === "pickup" ? "transparent" : alpha(theme.palette.text.primary, 0.2) }}
					>
						Pick-up (Surplus)
					</Button>
				</Box>
				{viewMode === "pickup" && (
					<Button
						...
						sx={{ height: 24, fontSize: "0.7rem", py: 0, px: 1, borderColor: enableSurplus ? "transparent" : alpha(theme.palette.text.primary, 0.2) }}
					>
			...
			{viewMode === "pickup" && !enableSurplus ? (
				...
				<>
					<FleetPlannerWidget
						...
						selectedMaterials={viewMode === "dropoff" ? selectedMaterials : selectedSurplusMaterials}
				...
				<Button
					size="small"
					startIcon={
						activeSelected.size === activeRows.length && activeRows.length > 0 ? (
							<CheckSquare size={16} />
						) : (
							<Square size={16} />
						)
				...
				>
					{activeSelected.size === activeRows.length && activeRows.length > 0 ? "None" : "All"}
				</Button>
				...
				<Stack spacing={1}>
					{activeRows.length > 0 ? (
						activeRows.map((row: any) => {
							...
							const allocated = activeCargoPlan.allocatedResults[row.ticker] || 0;
						...
						>
							{viewMode === "pickup" ? "No surplus production detected." : "No consumption detected."}
						</Typography>
					)}
				</Stack>
			</Box>
