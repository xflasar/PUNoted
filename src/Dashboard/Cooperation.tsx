...
const FlowManagerApp: React.FC = () => {
  ...
  const { staticData, isStaticDataLoading, staticDataError, forceRefetch } = useStaticData();
  ...
  const handleEditNode = useCallback((nodeData: ChainNodeData) => {
  ...
  const handleSaveEdit = (editedChainNodeData: ChainNodeData) => {
    ...
    const newRFNodes = currentRFNodes.map((n) =>
      n.nodeId === editedChainNodeData.nodeId 
        ? editedChainNodeData
        : n
    );
        ...
        >
          ...
          {staticDataError && !staticData && !isStaticDataLoading ? (
            ...
            >
              ...
              <Button variant="contained" color="primary" onClick={forceRefetch}>
                Retry Connection
              </Button>
            </Box>
          ) : isAppLoading ? (
            ...
            >
              ...
              <Typography variant="h6" color="text.secondary">
                {isStaticDataLoading ? "Loading Game Database..." : "Loading groups..."}
              </Typography>
            </Box>
          ) : selectedGroup ? (
            <Box
          ...
          <Box sx={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, pointerEvents: 'none', zIndex: 1200 }}>
            {staticData && (
              <>
                <NodeAdditionModal
                  open={isAddModalOpen}
