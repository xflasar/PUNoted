...
export const PriceWatcher = ({ exchange }: { exchange: string }) => {
	...
	const [watchers, setWatchers] = useState<WatcherItem[]>(() => {
		try {
			return JSON.parse(localStorage.getItem("cx_price_watchers") || "[]");
		} catch {
			return [];
		}
	});
	...
	useEffect(() => {
		localStorage.setItem("cx_price_watchers", JSON.stringify(watchers));
	}, [watchers]);
	...
	const saveEdit = (id: string) => {
		...
		if (!isNaN(val)) {
			...
			setWatchers((prev) =>
				prev.map((w) => {
					if (w.id === id) {
						const newTriggered = w.lastPrice
							? w.lastPrice > 0 && w.lastPrice <= val
							: false;
	...
	return (
		<Box sx={{ display: "flex", flexDirection: "column", height: "100%" }}>
			...
			>
				...
				<IconButton size="small" onClick={checkPrices} sx={{ p: 0.5 }}>
						...
						{watchers.length === 0 && (
							<TableRow>
								<TableCell
									colSpan={4}
									align="center"
									sx={{ color: "text.disabled", py: 2, fontSize: "0.8rem" }}
								>
									No active watchers
								</TableCell>
							</TableRow>
						...
						{watchers.map((w) => (
							...
							>
								...
								<TableCell
									...
									sx={{
										fontSize: "0.8rem",
										fontWeight: w.triggered ? "bold" : "normal",
										color: w.triggered ? "success.main" : "text.secondary",
									}}
								>
									{w.triggered ? (
										<Box
