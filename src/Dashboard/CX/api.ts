...
import type { CxDashboardStats, PaginatedCxOrders, PriceData, StorageValuationResponse } from "./types";
...
export const getDashboardStats = async (): Promise<CxDashboardStats> => {
    ...
    const response = await fetch(url, { method: 'GET' , headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${localStorage.getItem("authToken")}`}});
    if (!response.ok) throw new Error('Failed to fetch dashboard stats');
    const result = await response.json();
    if (!result.success) throw new Error('API returned an error for dashboard stats');
    return result.data;
};

export const getOrders = async (page: number, pageSize: number): Promise<PaginatedCxOrders> => {
...
export const getStorageValuation = async (exchangeTicker: string = 'IC1', storageId?: string | null): Promise<StorageValuationResponse> => {
    ...
    console.log("Fetching storage valuation for:", storageId || "Default", exchangeTicker);
...
export const getBulkPrices = async (tickers: string[], exchange: string = 'IC1'): Promise<PriceData[]> => {
