...
import { 
    Paper, Box, Typography, Collapse, IconButton, Divider, LinearProgress, useTheme 
} from '@mui/material';
...
export const LoanCard = ({ contract, type }: { contract: any, type: 'LENDER' | 'BORROWER' }) => {
        ...
        <Paper sx={{ 
            ...
            >
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                    ...
                    <Box>
                        <Typography variant="subtitle1" fontWeight={700}>{contract.name || "Loan Contract"}</Typography>
                        <Typography variant="caption" color="text.secondary">
                            {type === 'LENDER' ? `Lent to ${contract.partnername}` : `Borrowed from ${contract.partnername}`}
                        </Typography>
                    </Box>
                </Box>
                <Box sx={{ textAlign: 'right' }}>
                    ...
                    <Typography variant="caption" color={getStatusColor(contract.status, theme)}>
                        {contract.status}
                    </Typography>
                </Box>
            </Box>
            ...
            <Collapse in={expanded}>
                ...
                <Box sx={{ p: 2, bgcolor: alpha(theme.palette.background.default, 0.3) }}>
                    <Box sx={{ mb: 2 }}>
                        ...
                        <LinearProgress variant="determinate" value={progress} color={type === 'LENDER' ? 'success' : 'warning'} />
                    </Box>
                    
                    <Typography variant="caption" fontWeight="bold" sx={{ mb: 1, display: 'block' }}>CONDITIONS</Typography>
