...
import { 
    Box, Typography, Paper, useTheme, Stack, Divider, Select, MenuItem, ToggleButton, ToggleButtonGroup
} from '@mui/material';
...
const LoanOutlook = memo(({ loans }: LoanOutlookProps) => {
    ...
    const { graphData, weeklyData, profitStats } = useMemo(() => {
        ...
        let weekIn = 0, weekOut = 0; // Fixed 7 days for summary boxes
        const buckets: Record<string, { dateStr: string, Incoming: number, Outgoing: number }> = {};
    ...
    const handleRangeChange = (event: React.MouseEvent<HTMLElement>, newRange: number | null) => {
        if (newRange !== null) {
    ...
    return (
        <Paper elevation={0} sx={{ p: 2, mb: 2, bgcolor: alpha(theme.palette.background.default, 0.6), border: `1px solid ${theme.palette.divider}`, borderRadius: 2, width: '100%' }}>
            {/* Top Bar: Title & Controls */}
            <Stack direction="row" justifyContent="space-between" alignItems="center" mb={2} flexWrap="wrap" gap={2}>
                <Typography variant="subtitle2" fontWeight={800} color="text.secondary">CASHFLOW FORECAST</Typography>
                ...
                <Stack direction="row" spacing={1} alignItems="center">
                    ...
                    <ToggleButtonGroup 
                        ...
                        sx={{ height: 24 }}
                    >
                        <ToggleButton value={7} sx={{ fontSize: '0.7rem', py: 0 }}>7D</ToggleButton>
                        <ToggleButton value={14} sx={{ fontSize: '0.7rem', py: 0 }}>14D</ToggleButton>
                        <ToggleButton value={30} sx={{ fontSize: '0.7rem', py: 0 }}>30D</ToggleButton>
                    </ToggleButtonGroup>
                    <Select
                        value={customRange}
                        displayEmpty
                        ...
                        size="small"
                        renderValue={(sel) => sel === "" ? <Typography variant="caption">Custom</Typography> : <Typography variant="caption">{sel} Days</Typography>}
                        sx={{ height: 24, minWidth: 80, fontSize: '0.7rem' }}
                    >
                        {[60, 90, 180].map(d => <MenuItem key={d} value={d}>{d} Days</MenuItem>)}
                    </Select>
                </Stack>
            </Stack>
            ...
            <Stack direction={{ xs: 'column', md: 'row' }} spacing={3} alignItems="stretch">
                {/* Summary Boxes (Left) */}
                <Box sx={{ width: { xs: '100%', md: 240 }, flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 1 }}>
                    <Box sx={{ p: 2, bgcolor: alpha(theme.palette.background.default, 0.5), borderRadius: 2, border: `1px solid ${alpha(theme.palette.divider, 0.5)}` }}>
                        <Typography variant="caption" fontWeight={700} color="text.secondary" mb={1} display="block">NEXT 7 DAYS SUMMARY</Typography>
                        <Stack direction="row" justifyContent="space-between" alignItems="center" mb={1}>
                            <Box>
                                <Typography variant="caption" display="block" color="success.main" fontWeight={700}>INCOMING</Typography>
                                <Typography variant="h6" fontWeight={700} lineHeight={1}>{formatCurrency(weeklyData.in, 'ICA')}</Typography>
                            </Box>
                        ...
                        <Stack direction="row" justifyContent="space-between" alignItems="center">
                            <Box>
                                <Typography variant="caption" display="block" color="error.main" fontWeight={700}>OUTGOING</Typography>
                                <Typography variant="h6" fontWeight={700} lineHeight={1}>{formatCurrency(weeklyData.out, 'ICA')}</Typography>
                            </Box>
                ...
                <Box sx={{ flexGrow: 1, minHeight: 220 }}>
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={graphData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                            <CartesianGrid strokeDasharray="3 3" vertical={false} stroke={alpha(theme.palette.text.disabled, 0.1)} />
                            <XAxis 
                                dataKey="dateStr" 
                            ...
                            <Bar dataKey="Incoming" fill={theme.palette.success.main} radius={[4, 4, 0, 0]} barSize={20} />
                            <Bar dataKey="Outgoing" fill={theme.palette.error.main} radius={[4, 4, 0, 0]} barSize={20} />
                        </BarChart>
                    </ResponsiveContainer>
                </Box>
            </Stack>
