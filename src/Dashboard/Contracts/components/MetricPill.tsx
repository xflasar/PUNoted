...
const MetricPill = memo(({ label, value, sub, color }: MetricPillProps) => {
    ...
    return (
        <Paper elevation={0} sx={{ 
            ...
            <Typography variant="caption" color="text.secondary" fontWeight={800} fontSize="0.75rem" letterSpacing={1} display="block" sx={{ mb: 0.5, textTransform: 'uppercase' }}>
                {label}
            </Typography>
            <Typography variant="h5" fontWeight={800} lineHeight={1.2} color="text.primary">
                {typeof value === 'number' ? formatCurrency(value, 'ICA') : value}
            </Typography>
            {sub && <Typography variant="caption" fontSize="0.7rem" color="text.secondary" sx={{ mt: 0.5, display: 'block' }}>{sub}</Typography>}
        </Paper>
    );
});
