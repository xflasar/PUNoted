...
const StatCard = ({ title, value, subValue, icon, color, theme }: any) => (
    <Paper sx={{ 
        ...
        <Box>
            ...
            {subValue && <Typography variant="caption" color={subValue.includes('-') ? 'error.main' : 'success.main'}>{subValue}</Typography>}
        </Box>
    </Paper>
);
...
const ContractStats: React.FC<Props> = ({ stats, loading }) => {
    ...
    if (loading || !stats) return <Box sx={{ height: 200, bgcolor: alpha(theme.palette.action.disabled, 0.1), borderRadius: 2 }} />;

    const chartData = Object.entries(stats.status_counts).map(([name, value]) => ({ name, value }));
    const COLORS = [theme.palette.success.main, theme.palette.info.main, theme.palette.warning.main, theme.palette.error.main, theme.palette.text.disabled];
                ...
                <Paper sx={{ 
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={chartData}>
                            <XAxis dataKey="name" tick={{ fontSize: 10, fill: theme.palette.text.secondary }} interval={0} />
                            <Tooltip 
